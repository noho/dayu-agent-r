...
| RR-LAYER-02-01 | WU-LAYER-02 aggregate review / PR review | `llm_compaction._safe_outcome_text` 截断形状与 runtime truncate 语义不同 | deferred-with-owner | future Host compactor diagnostic hardening if unified truncation is desired | 当前 PR 保留旧 Host-specific 截断形状；若未来要统一 Host compactor diagnostic 截断规则，先做显式行为决策并更新对应测试 | `_safe_outcome_text` 超限时保留旧行为：前 240 个字符再追加 `"..."`，结果总长可到 243；runtime `truncate_diagnostic_text` 是总长不超过 `max_chars`。该差异已由 controller 裁决和测试锁定，不影响安全性或当前 PR。 |
