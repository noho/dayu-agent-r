...
| gate | implementation |
| implementation status | WU-DUR-01 + WU-DUR-02 已完成：draft-PR-pass；WU-LIFE-01 + WU-LIFE-02 已完成：draft-PR-pass；WU-CTX-02 + WU-CTX-03 accepted Slice C commit: 2d2ed5a，下一步进入 Slice D |
| active work unit | WU-CTX-02 + WU-CTX-03 |
| default next work unit | WU-CTX-02 + WU-CTX-03 |
| next entry point | WU-CTX-02 + WU-CTX-03 Slice D implementation；WU-LIFE-01 + WU-LIFE-02 merge / ready-for-review / reviewer request require separate authorization；WU-DUR-01 + WU-DUR-02 merge / ready-for-review / reviewer request require separate authorization |
...
| RR-CTX-SLICEB-01 | WU-CTX-02 + WU-CTX-03 Slice B code review | reactive `context_budget_policy_missing` / `input_event_missing` precondition 集成覆盖 | deferred-with-owner | WU-CTX Slice D / aggregate review | 若 Slice D 触及 reactive precondition / fallback failure，再评估是否补这两条边缘路径集成测试；aggregate review 再复查是否仍需 hardening | Slice B 已通过统一 helper 与 payload builder 覆盖核心字段；这两条边缘路径不阻塞 Slice B。 |
