...
| gate | draft-PR-pass |
| implementation status | WU-CM-12 accepted plan commit `8186f678`; implementation Slice S1 accepted in commit `7f71c731`; implementation Slice S2 accepted in commit `7b239aef`; implementation Slice S3 accepted in commit `3bab485c`; implementation Slice S4 accepted in commit `c12e9952`; implementation Slice S5 accepted in commit `2c46631b`; aggregate deepreview and final closeout passed locally; draft PR #150 opened |
| active work unit | WU-CM-12 |
| default next work unit | WU-CM-12 |
| next entry point | WU-CM-12 draft PR #150 is open; merge / ready-for-review / reviewer requests require explicit user authorization. |
...
| WU-CM-12-FC-R1 | deferred-with-owner | WU-CM-14 Accepted tool evidence material governance follow-up | Final closeout constant audit found `_ACCEPTED_TOOL_EVIDENCE_MATERIAL_LIMIT` remains a RunInput accepted-tool-evidence row-count guard outside `memory_projection_policy`. It does not truncate text and is outside WU-CM-12 Conversation Memory policy-owner repair scope, but future accepted-tool-evidence material governance must assign WU-CM-14 by user or GitHub Issue before changing it. |
...
| WU-CM-12 | draft-PR-pass | Conversation Memory design refinement and implementation drift repair | 用户裁决；无 GitHub Issue | Pre-plan design truth repair and accepted plan completed; implementation S1/S2/S3/S4/S5 accepted in `7f71c731` / `7b239aef` / `3bab485c` / `c12e9952` / `2c46631b`; aggregate deepreview PASS; final closeout completed locally; draft PR #150 open. |
...
| WU-CM-14 | deferred | Accepted tool evidence material governance follow-up | WU-CM-12-FC-R1 follow-up；无 GitHub Issue | Deferred destination only. WU-CM-12 did not change accepted tool evidence retrieval-volume policy. `_ACCEPTED_TOOL_EVIDENCE_MATERIAL_LIMIT` is a row-count guard, not text truncation, but future governance should decide whether accepted tool evidence material quantity belongs in a typed policy before implementation. |
...
- final closeout residuals: `WU-CLI-ACTIVITY-01-PR-R1` and `WU-CM-12-S1-R1` closed; `WU-CM-12-S4-R1` deferred to WU-CM-13; accepted tool evidence material retrieval-volume audit item `WU-CM-12-FC-R1` deferred to WU-CM-14.
- draft PR gate: branch `wu-cm-12-conversation-memory-drift` pushed to `github`; draft PR #150 opened at https://github.com/noho/dayu-agent-r/pull/150; `gh pr checks 150` reported no checks on the branch at creation time.
