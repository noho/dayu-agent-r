...
| implementation status | WU-CM-13 Slice 2c code review PASS. RunInput ordinary post-compaction protected raw-tail selection now delegates to pipeline-owned `select_ordinary_protected_raw_tail(...)`; the durable second-read adapter remains freshness/caller-owned but no longer owns protected group eligibility or memory dedup. Accepted slice commit is pending. |
| active work unit | WU-CM-13 |
| default next work unit | WU-CM-13 |
| next entry point | Create the WU-CM-13 Slice 2c accepted slice commit, then update this control doc with the commit hash and enter aggregate deepreview. Aggregate deepreview must include the final smoke hard gate reminder: `utils/smoke_host_public_conversation_memory_scenarios.py` must pass without modifying the smoke. |
...
| blocking open questions | None for WU-CM-13 accepted slice commit. WU-CLI-DEBUG-STREAM-01 still needs discussion / plan confirmation before code changes. Draft PR #150 remains open draft; merge, mark-ready, reviewer requests, and issue closure require separate user authorization. |
...
| WU-CM-13 | implementation | Unified conversation compact pipeline convergence | WU-CM-12-S4-R1 follow-up；无 GitHub Issue | Slice 2c code review PASS. Proactive/reactive compact request, recovery, fallback decision input, and ordinary post-compaction raw-tail selection now use shared compact pipeline helpers; next gate is accepted slice commit followed by aggregate deepreview. |
...
- next gate: accepted Slice 2c commit.
