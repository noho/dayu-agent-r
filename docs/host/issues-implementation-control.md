...
| gate | implementation |
| implementation status | WU-CM-01-F01-S7-R1 plan accepted; S7-R1-S0 design contract sync ready |
| active work unit | WU-DUR-P01 / WU-OBS-P00 / WU-CM-01-F02 / WU-CM-01-F01 |
| default next work unit | WU-CM-01-F01 |
| next entry point | implement S7-R1-S0 design contract sync before production `run_input.py` changes |
...
| WU-CM-01-F01-S7-R1 | WU-CM-01-F01 Slice 7 retry blocker | public runner-call multi-system-message shape | open | WU-CM-01-F01 production RunInput / memory projection rescope | Implement S7-R1-S0 design sync, then production one-system-message assembly contract after design review. | Slice 7 retry assertions read actual public `AgentRunRequest.messages` / runner `messages_seen` and found 2-5 system messages on ordinary public runner calls; accepted plan review findings require concrete section titles/separators, selected evidence trade-off, internal ref replacement table, manifest test boundary, and boundedness sanity before code. |
...
| WU-CM-01-F01 | implementation-S7-R1-S0-design-sync | Conversation Memory smoke correctness closeout | GitHub Issue #81 / WU-CM-01 final closeout | S7-R1 plan accepted; next gate updates design truth before production `run_input.py` changes |
