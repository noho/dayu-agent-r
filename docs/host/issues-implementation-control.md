...
| gate | implementation |
| implementation status | WU-CM-01-F01 Slice 7 implementation-ready |
| active work unit | WU-DUR-P01 / WU-OBS-P00 / WU-CM-01-F02 / WU-CM-01-F01 |
| default next work unit | WU-CM-01-F01 |
| next entry point | AgentCodex implements Slice 7 public smoke correctness closeout |
...
| current inspection note | 2026-06-05 phaseflow resumed after WU-CM-01 PR merge. Goal confirmation accepted the dependency chain WU-DUR-P01 -> WU-OBS-P00 -> WU-CM-01-F02 -> WU-CM-01-F01, with WU-CM-01-F01 as public smoke validation for the first three WU. Accepted plan commit is c1e9de3f. Slice 0 accepted commit is 83cf38d8. Slice 1 accepted commit is 82eb5813. Slice 2 accepted commit is 113c2838. Slice 3 accepted commit is bea9f488. Slice 4 accepted commit is 11688014. Slice 5 accepted commit is 104e10bb. Slice 6 accepted commit is f6474b0c. Slice 6 rewrote default compactor prompts to remove internal implementation / migration terminology, added prompt self-containment assertions, and fixed evidence retention wording back to strong `must` semantics. Final Slice 6 validation passed: 6 public compact smoke tests passed, 1 skipped; pyright 0 errors; git diff check clean. Active residual `WU-CM-01-F02-S6-R1` tracks runtime `instruction.output_schema_name` still exposing `ConversationCompactOutputVNext` in LLM-facing material JSON; owner is production compact instruction contract rescope before WU-CM-01-F01 final public smoke acceptance. Deferred risks `WU-DUR-P01-S2-R2`, `WU-DUR-P01-S3-R1` and `WU-DUR-P01-S3-R2` remain tracked with owner. Next gate is WU-CM-01-F01 Slice 7 implementation for public smoke correctness closeout. |
...
|---|---|---|---|---|---|---|
| WU-DUR-P01-S2-R2 | WU-DUR-P01 Slice 2 re-review | runner-call matching hardening | deferred-with-owner | WU-DUR-P01 later runner-call hardening or WU-OBS-P00 / Slice 4 | Harden or explicitly test `_runner_call_manifest_matches_iteration` if Engine iteration indexing semantics change, especially continuation scenarios with reset `iteration_index == 0`. | Current Engine iteration indexes are monotonic within an execution; normal continuation path is covered and does not mis-match the ordinary first-call manifest. |
| WU-DUR-P01-S3-R1 | WU-DUR-P01 Slice 3 code review | trigger reason enum precision | deferred-with-owner | WU-DUR-P01 design contract hardening before WU-OBS-P00 analyzer consumption | Add or otherwise document an initial compactor proposal trigger reason instead of labeling attempt 1 as repair/retry. | Current manifest uses a legal enum value but the design enum lacks a precise initial proposal value; fixing it correctly requires design contract update, not this narrow implementation fix. |
| WU-DUR-P01-S3-R2 | WU-DUR-P01 Slice 3 code review | outcome cross-reference contract | deferred-with-owner | WU-OBS-P00 / WU-DUR-P01 design contract cleanup | Clarify whether outcome-dependent accepted/rejected event refs live in compact event payloads or require an explicit post-outcome manifest update contract. | Proposal manifest is written before runner call, so accepted/rejected event refs do not exist at manifest write time; current compact payloads provide reverse refs to the proposal manifest. |
| WU-CM-01-F02-S6-R1 | WU-CM-01-F02 Slice 6 code review | LLM-facing compact instruction literal | open | WU-CM-01-F02 production compact instruction contract rescope before WU-CM-01-F01 final public smoke acceptance | Replace or otherwise remove runtime `instruction.output_schema_name = "ConversationCompactOutputVNext"` from the final LLM-facing compaction request, with design/source-label/schema validation updated as needed. | Slice 6 prompt templates are clean, but DS review found the runtime material JSON still injects the Python type name through `CompactInstructionVNext`; fixing it requires production contract changes outside Slice 6 prompt allowed files. |
...
| WU-CM-01-F01 | implementation-slice-7 | Conversation Memory smoke correctness closeout | GitHub Issue #81 / WU-CM-01 final closeout | Slice 7 implementation entry point: public smoke correctness closeout validates runner-call input manifest, compact query readability, prompt semantics, and records any production contract blocker before final acceptance |
| WU-CM-01-F02 | completed-with-residual | Compact evidence query readability quality closeout | GitHub Issue #81 / WU-CM-01 final closeout；depends on WU-DUR-P01 durable tool-call atoms | Slice 5 / Slice 6 accepted; active residual `WU-CM-01-F02-S6-R1` must be resolved or explicitly resliced before WU-CM-01-F01 final public smoke acceptance |
...
| WU-DUR-P01 | completed-with-residuals | EventLog runner-call reconstruction atoms | GitHub Issue #117 | Slice 0-3 accepted; remaining residuals have owners in active residual risk table |
