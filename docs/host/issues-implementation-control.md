...
| next entry point | Repair WU-CM-12 open fix `WU-CM-12-FIX-R1`: remove over-designed compact input field-length guards / default evidence chunking / private accepted-tool-evidence row limit from EventLog-derived LLM input material paths, then rerun review and final closeout. |
