...
| next entry point | Implementation gate for WU-TOOLS-CANCEL-01 S2A2 `Host factory wiring` on branch `phase/wu-tools-cancel-01`. Implement declaration-backed Host capsule factory wiring from `ToolDefinition.execution`, add `BatchToolExecutionContext -> ProcessBackedToolContext` projection tests, keep Engine projection free of execution capability, and stop if Host wiring requires tool-name branching, Engine contract changes, or changing `dayu.runtime.interruptible_process` away from `JsonValue`. |
...
| blocking open questions | None for S2A2 entry. User confirmed WU-TOOLS-CANCEL-01 goal: #87 closeout requires Codex / Claude Code style Esc interrupt user experience, with WU-LIFE-03 / WU-LIFE-04 / WU-WAIT-03 consumed as completed prerequisites. |
...
| WU-TOOLS-CANCEL-01 | implementation | Tool/provider blocking I/O cancellation hardening | follows WU-LIFE-04 | WU-LIFE-04 已完成；accepted plan commit 为 `4723ec61`，S1 accepted slice commit 为 `eda4be1a`，S2 partial hardening commit 为 `29003541`，typed execution capability plan commit 为 `8eddd26b`。S2A1 `contract / declaration / digest` 已通过 AgentCodex implementation、AgentMiMo / AgentDS review、advisory fix、AgentMiMo / AgentDS re-review 与 controller adjudication；当前下一步为 S2A2 `Host factory wiring` implementation gate。S2A2 必须从 `ToolDefinition.execution` 创建 declaration-backed capsule factory，投影 `ProcessBackedToolContext` 时不携带 `cancellation_token`，保持 Engine 不消费 execution capability，并不得修改 `dayu.runtime.interruptible_process` 的 `JsonValue` 契约。生产级目标仍是 Codex / Claude Code 类似 interrupt 体感：取消后 Host 迅速回到可交互状态，旧结果不能污染已取消 Run，且不得延长单次工具执行 deadline；不可抢占 blocking I/O 必须迁移到可中断 capsule 或被禁止进入 production cancel path。 |
