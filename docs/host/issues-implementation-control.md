...
| WU-CM-12-PR-R2 | deferred-with-owner | Future compact / memory render defensive-depth governance WU；no default next entry | PR review observed `_vnext_compact_candidate_semantic_lines` renders accepted compact fact text after parser acceptance while memory projection also has policy caps. Current code remains compliant because parser safety guards bound LLM-generated output candidates and no EventLog-derived input material is truncated; future governance may document or align this defensive-depth asymmetry. |
...
- WU-CM-12-FIX-R1 PR review adjudication: core FIX-R1 material-guard objective PASS in both reviews. Accepted and fixed only local quality findings for fallback `current_input_ref` diagnostic ordering and fallback selected-window cap boundary tests. `compaction_evidence.py` cleanup, `_vnext_compact_candidate_semantic_lines` defensive-depth asymmetry, and recovery-tier rejected-attempt diagnostic completeness remain deferred / non-blocking residuals for later cleanup or governance owner; `_facts_from_accepted_event` old bug fix must be called out in final closeout.
