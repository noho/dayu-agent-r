...
| gate | remediation accepted-plan commit |
...
| next entry point | Controller accepted-plan commit gate. The final umbrella remediation plan, design/controller truth, review chain, and control state have passed two complete re-review routes with no accepted finding open. Controller must create the authorized local accepted-plan commit from the explicit current-WU artifact set, then record its SHA and enter R01 internal remediation sub-WU plan gate. This does not authorize R01 implementation or skip R01's independent plan/review/fix/re-review/accepted-plan-commit sequence. |
