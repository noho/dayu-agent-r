...
| gate | R01-S1 accepted local commit pending |
...
| next entry point | Controller performs final diff/status verification and creates the authorized R01-S1 accepted local commit containing the complete source snapshot implementation, tests and gate artifacts. After the commit, controller records the accepted hash and enters only the accepted R01-S2 directory completeness implementation gate; R01 as a whole remains open. |
