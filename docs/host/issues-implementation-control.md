...
| gate | implementation |
| implementation status | WU-TOOLS-CANCEL-01 accepted plan / S1 / S2 partial hardening / typed execution plan are committed. S2A1 `contract / declaration / digest`, S2A2 `Host factory wiring`, S2B `Doc process-backed`, and S2C `Fins read process-backed` have passed implementation, dual review, fix where accepted, dual re-review, controller validation, and controller adjudication. Latest accepted slice is S2C; current entry point is S2D `Web sync process-backed or abort-capable async_direct`. |
...
| next entry point | Implementation gate for WU-TOOLS-CANCEL-01 S2D `Web sync process-backed or abort-capable async_direct` on branch `phase/wu-tools-cancel-01`. Migrate Web search / fetch synchronous requests paths away from uninterruptible same-process `asyncio.to_thread(...)` into `process_backed`, or replace with request-abort-capable `async_direct` with explicit response/client/stream close verification. Playwright path must keep process-backed / fail-closed semantics and must not fall back to an uninterruptible same-process path. Timeout budget must continue to flow into HTTP / browser stages. Stop if Web cannot provide process-backed execution or a proven abort-capable async direct path, if cancellation cannot prove no late accepted result, or if the change requires Engine contract / durable schema changes. |
...
| blocking open questions | None for S2D entry. User confirmed WU-TOOLS-CANCEL-01 goal: #87 closeout requires Codex / Claude Code style Esc interrupt user experience, with WU-LIFE-03 / WU-LIFE-04 / WU-WAIT-03 consumed as completed prerequisites. |
...
| WU-TOOLS-CANCEL-01 | implementation | Tool/provider blocking I/O cancellation hardening | follows WU-LIFE-04 | WU-LIFE-04 已完成；accepted plan commit 为 `4723ec61`，S1 accepted slice commit 为 `eda4be1a`，S2 partial hardening commit 为 `29003541`，typed execution capability plan commit 为 `8eddd26b`。S2A1 `contract / declaration / digest`、S2A2 `Host factory wiring`、S2B `Doc process-backed` 和 S2C `Fins read process-backed` 均已通过 implementation、AgentMiMo / AgentDS review、accepted finding fix、AgentMiMo / AgentDS re-review、controller validation 与 controller adjudication。S2C 通过 `ToolDefinition.execution` 为九个非 WAITING Fins read tools 声明 process-backed target factory，子进程通过 `DefaultFinsRuntime` / `dayu.fins.storage` 重新打开只读仓储，测试覆盖 fast path、processor path、table path、FinancialDataProcessor spawned-child path、failed envelope 和 ToolRuntime cancel late-result drop。当前下一步为 S2D `Web sync process-backed or abort-capable async_direct` implementation gate：Web search / fetch 同步 requests 路径必须迁移到可中断 process-backed，或替换为有 response / client / stream close 证据的 abort-capable async direct；Playwright 路径必须保持 process-backed / fail-closed，不得回落到不可抢占同进程路径。生产级目标仍是 Codex / Claude Code 类似 interrupt 体感：取消后 Host 迅速回到可交互状态，旧结果不能污染已取消 Run，且不得延长单次工具执行 deadline；不可抢占 blocking I/O 必须迁移到可中断 capsule 或被禁止进入 production cancel path。 |
