...
| gate | R01-S1 complete SourceSnapshot and source-limit removal implementation |
...
| next entry point | AgentCodex implements only accepted-plan Slice `R01-S1`: complete `SourceSnapshot` and source-byte-limit contract removal. It must stay inside the S1 production/test/artifact allowlist, run all S1 owner/consumer tests, per-file coverage, pyright and scans, and stop before R01-S2 or code review. |
