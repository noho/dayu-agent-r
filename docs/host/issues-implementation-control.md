...
| next entry point | Implementation gate for WU-TOOLS-CANCEL-01 S2A1 `contract / declaration / digest` on branch `phase/wu-tools-cancel-01`. Implement the accepted typed execution capability contract in `dayu.contracts`, update `ToolDefinition` / decorator / direct construction sites / ToolsDiscovery digest, keep `dayu.runtime.interruptible_process` on the `JsonValue` contract, and stop if the S2A1 stop conditions in `docs/host/wu-tools-cancel-01-typed-execution-capability-plan.md` are hit. |
...
| blocking open questions | None for S2A1 plan entry. User confirmed WU-TOOLS-CANCEL-01 goal: #87 closeout requires Codex / Claude Code style Esc interrupt user experience, with WU-LIFE-03 / WU-LIFE-04 / WU-WAIT-03 consumed as completed prerequisites. |
...
| WU-TOOLS-CANCEL-01 | implementation | Tool/provider blocking I/O cancellation hardening | follows WU-LIFE-04 | WU-LIFE-04 已完成；accepted plan commit 为 `4723ec61`，S1 accepted slice commit 为 `eda4be1a`，S2 partial hardening commit 为 `29003541`。typed execution capability design/contract plan 已通过 AgentMiMo / AgentDS plan review、AgentCodex plan fix、AgentMiMo / AgentDS re-review 与 controller adjudication；当前进入 S2A1 `contract / declaration / digest` implementation gate。S2A1 必须保持 capability 真源在 `dayu.contracts.ToolDefinition.execution`，保持 `dayu.runtime.interruptible_process` 层中立 `JsonValue` 契约，补全直接 `ToolDefinition` 构造站点与 ToolsDiscovery digest，并满足 plan 中 process target/context/envelope pickle round-trip、thread_backed guard 与 stop conditions。生产级目标仍是 Codex / Claude Code 类似 interrupt 体感：取消后 Host 迅速回到可交互状态，旧结果不能污染已取消 Run，且不得延长单次工具执行 deadline；不可抢占 blocking I/O 必须迁移到可中断 capsule 或被禁止进入 production cancel path。 |
