...
| next gate | push专用分支并创建draft PR；创建成功后next gate立即切换为PR review。 |
...
| next entry point | accepted deepreview commit=`0a72396d`；Controller push `phaseflow/wu-host-session-event-delivery-01`并创建不关联Issue的draft PR。不得mark ready、merge、request reviewers、关闭外部Issue或delete branch。 |
...
| WU-HOST-SESSION-EVENT-DELIVERY-01 | active-push | Host Session Event Delivery ownership、bounded mailbox 与跨 opener ordering | 用户明确裁决；无 GitHub Issue | accepted deepreview commit=`0a72396d`；进入push与create draft PR。 |
