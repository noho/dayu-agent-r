...
| implementation status | WU-CM-13 aggregate deepreview PASS. Proactive dispatch, reactive ingest, fallback decision input, and ordinary post-compaction protected raw-tail selection share compact pipeline helpers while lifecycle guards remain caller-owned; final `memory-compact` smoke passed without modifying the smoke. Accepted deepreview commit is pending. |
| active work unit | WU-CM-13 |
| default next work unit | WU-CM-13 |
| next entry point | Create the WU-CM-13 accepted deepreview commit, then update this control doc with the commit hash and move to ready-to-open-draft-PR / PR workflow decision. |
...
| blocking open questions | None for WU-CM-13 accepted deepreview commit. WU-CLI-DEBUG-STREAM-01 still needs discussion / plan confirmation before code changes. Draft PR #150 remains open draft; merge, mark-ready, reviewer requests, and issue closure require separate user authorization. |
...
| WU-CM-13 | implementation | Unified conversation compact pipeline convergence | WU-CM-12-S4-R1 follow-up；无 GitHub Issue | Aggregate deepreview PASS. Final `memory-compact` smoke passed; `WU-CM-13-S1-R1` closed; next gate is accepted deepreview commit. |
...
- aggregate residual reconciliation: `WU-CM-13-S1-R1` closed. The old malformed compacted payload fact-ref edge is closed by the typed `ConversationCompactOutputVNext` helper boundary plus operation-level candidate rejection coverage and compact payload/material provenance tests.
