...
| awaiting poller follow-up | User-reported `dayu-cli interactive` Fins download completion left CLI waiting is closed locally pending user real-environment re-validation. Implementation artifact: `docs/reviews/wu-cli-smoke-01-awaiting-poller-fix-codex.md`; review artifacts: `docs/reviews/wu-cli-smoke-01-awaiting-poller-review-mimo.md` and `docs/reviews/wu-cli-smoke-01-awaiting-poller-review-ds.md`; controller adjudication: `docs/reviews/wu-cli-smoke-01-awaiting-poller-controller-adjudication.md`. The root cause was that product entrypoint runtime exposed Fins awaiting tools but left `ServiceAssemblyOverrides.wait_poller_policy=None`, so `open_host` did not start the production wait poller even though `HostToolingOptions.wait_poll_adapter_registry` was available. The fix adds `with_entrypoint_wait_poller_policy(...)` in Service assembly and calls it from `prepare_entrypoint_runtime(...)`, enabling production wait poller only when actual scene tool selection exposes Fins download / preprocess / upload awaiting tools. Host default no-poller contract remains unchanged, and Fins completion still returns through `FinsIngestionWaitPollAdapter` into `resolve_wait`; Fins tools do not directly notify Host. Controller validation passed affected tests (`157 passed`), pyright (`0 errors`), and `git diff --check`; AgentCodex also reported broader Service / CLI tests (`265 passed`). Two-agent review found no blocker. |
...
| next entry point | Await user manual validation feedback for prompt / interactive behavior, current-time/tool boundary, and Tool Trace auditability, then enter final closeout for this context slot / tool trace follow-up if no new blocker is found. |
