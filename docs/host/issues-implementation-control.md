...
| WU-TOOLS-01-F03-R4-PATH-R1 | deferred-with-owner | Future Doc provider path-boundary hardening | Doc provider 当前对 configured `allowed_paths` 和工具入参都使用 `Path.resolve(...)` 后再判断是否位于 allowed root 内；这意味着 symlink 会按真实目标路径裁决。本 WU 不改变该策略，后续如需不 follow symlink 或加入更细路径策略，应在 Doc provider path-boundary hardening 中单独裁决。 |
...
- `deferred-with-owner`：`WU-TOOLS-01-F03-R4-PATH-R1` tracks future Doc provider path-boundary hardening if symlink policy needs to differ from current `Path.resolve()` semantics.
