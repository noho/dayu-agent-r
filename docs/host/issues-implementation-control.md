...
| implementation status | WU-CM-13 Slice 2c accepted commit `7aab0f94`. Proactive dispatch, reactive ingest, fallback decision input, and ordinary post-compaction protected raw-tail selection now share compact pipeline helpers while lifecycle guards remain caller-owned. Next gate is aggregate deepreview across WU-CM-13. |
| active work unit | WU-CM-13 |
| default next work unit | WU-CM-13 |
| next entry point | Dispatch WU-CM-13 aggregate deepreview over accepted commits `0390c9ad`, `b180a510`, `7b0367ab`, and `7aab0f94`. Aggregate review must audit proactive/reactive shared helper convergence, WU-CM-14 preservation integration, residual `WU-CM-13-S1-R1`, LLM-facing provenance boundaries, no tier 5 / `fallback_tier` regression, and remind that final acceptance requires `utils/smoke_host_public_conversation_memory_scenarios.py` to pass without modifying the smoke. |
...
| blocking open questions | None for WU-CM-13 aggregate deepreview. WU-CLI-DEBUG-STREAM-01 still needs discussion / plan confirmation before code changes. Draft PR #150 remains open draft; merge, mark-ready, reviewer requests, and issue closure require separate user authorization. |
...
| WU-CM-13 | implementation | Unified conversation compact pipeline convergence | WU-CM-12-S4-R1 follow-up；无 GitHub Issue | Slice 2c accepted commit `7aab0f94`. Proactive/reactive compact request, recovery, fallback decision input, and ordinary post-compaction raw-tail selection now use shared compact pipeline helpers; next gate is aggregate deepreview. |
