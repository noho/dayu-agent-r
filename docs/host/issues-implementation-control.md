...
| next entry point | WU-CM-13 is ready for PR workflow decision: choose whether to push `wu-cm-14-final-answer-preservation` and create a new draft PR for WU-CM-14 + WU-CM-13, or update an existing draft PR if the user explicitly assigns that target. |
...
| blocking open questions | PR target requires user decision before external side effects: create a new draft PR for WU-CM-14 + WU-CM-13, or update an explicitly assigned existing draft PR. WU-CLI-DEBUG-STREAM-01 still needs discussion / plan confirmation before code changes. Merge, mark-ready, reviewer requests, and issue closure require separate user authorization. |
...
| WU-CM-13 | ready-to-open-draft-PR | Unified conversation compact pipeline convergence | WU-CM-12-S4-R1 follow-up；无 GitHub Issue | Accepted deepreview commit `00da03a3`. Final `memory-compact` smoke passed; `WU-CM-12-S4-R1` and `WU-CM-13-S1-R1` closed; next gate is PR workflow decision. |
...
- next gate: ready-to-open-draft-PR / PR workflow decision.
