...
| next entry point | Dispatch a narrowed MANUAL-F01 root-cause fix gate for interactive Agent `start_fins_download` awaiting path versus direct `dayu-cli download --ticker V`. |
...
| blocking open questions | MANUAL-F01 blocks final closeout: interactive Agent `start_fins_download` awaiting path fails with HostDurableError while direct `dayu-cli download --ticker V` starts progress. MANUAL-01 and MANUAL-02 are accepted for provider prompt transport and running-state Ctrl+C; MANUAL-03 remains optional unless the user widens scope. |
