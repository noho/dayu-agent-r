...
| next entry point | WU-LIFE-03 / GitHub Issue #91 draft PR gate after aggregate deepreview pass. |
...
| WU-LIFE-03 | ready-to-open-draft-PR | Active cancel watchdog | GitHub Issue #91 / #87 umbrella | Aggregate deepreview 已通过；当前进入 draft PR gate。固定 Host-level active cancel watchdog、post-cancel timeout closeout、late terminal race 和 diagnostic 语义。只负责 Host truth / timeout closeout，不负责 tool/provider hard interrupt。 |
