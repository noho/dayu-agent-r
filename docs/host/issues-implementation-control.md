...
| WU-CM-01-PR-D1 | WU-CM-01 PR review | public surface hardening | deferred-with-owner | WU-CM-01 post-PR cleanup / Host public surface hardening | 视后续 public surface cleanup 决定是否为 `dayu/host/memory.py` 与 `dayu/host/context_fallback.py` 增加 `__all__` | `docs/reviews/wu-cm-01-pr-review-controller-adjudication.md` D-1；不影响当前 PR correctness、contract 或 schema。 |
| WU-CM-01-PR-D2 | WU-CM-01 PR review | typed cleanup | deferred-with-owner | compact operation typed cleanup follow-up | 评估是否将 `CompactionAttemptRejected.failure_category` 与 `next_policy_decision` 从 `str` 收紧为 enum | `docs/reviews/wu-cm-01-pr-review-controller-adjudication.md` D-2；当前字段由模块内常量集中写入，review 未给出错误数据路径。 |
| WU-CM-01-PR-D4 | WU-CM-01 PR review | diagnostic naming cleanup | deferred-with-owner | Host diagnostic string cleanup follow-up | 后续清理 module-private `slice1` diagnostic 常量命名 | `docs/reviews/wu-cm-01-pr-review-controller-adjudication.md` D-4；不影响外部 contract。 |
| WU-CM-01-PR-D5 | WU-CM-01 PR review | evaluation / test hardening | deferred-with-owner | GitHub Issue #80 / Conversation Memory evaluation 与后续 Host test hardening | 后续补 large evidence chunk、repair 集成测试、quality gate 拒绝路径、fallback path 与并发矩阵覆盖 | `docs/reviews/wu-cm-01-pr-review-controller-adjudication.md` D-5；当前 PR 已通过 targeted tests、pyright、full Host test 与 public smoke。 |
