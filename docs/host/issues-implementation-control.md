...
| WU-DUR-P01-S3-R1 | WU-DUR-P01 Slice 3 code review | trigger reason enum precision | closed | WU-DUR-P01 residual implementation | Added `context_compaction_initial_proposal` to `RunnerCallTriggerReason` and changed first compactor proposal manifests to use it instead of repair/retry reasons. | Focused compaction operation test covers first proposal vs retry trigger reasons; public compact smoke asserts the durable manifest trigger reason. |
