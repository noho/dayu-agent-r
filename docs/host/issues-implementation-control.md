...
| gate | R02 accepted-plan local commit |
...
| next entry point | Controller creates the R02 accepted-plan local commit containing only the final plan, plan-entry adjudication, complete initial review/adjudication/fix/re-review chain and this accepted control state. After that commit, Controller separately transitions the control document to R02-S1 implementation and assigns only S1 to AgentCodex. S2/S3, R03, Issue 178 lifecycle and unified authorization remain unauthorized. |
