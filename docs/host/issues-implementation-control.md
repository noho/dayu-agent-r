...
| next entry point | R03-S2 accepted local commit is `4b4696e5` (`gateflow: accept R03 S2 LLM source owners`); all S2 findings are closed. The current gate is AgentCodex R03-S3 implementation under accepted plan §11: opaque refs remain internal provenance, explicit citation is the sole readable business source, RunInput/Memory/Compact/LLM-ready Tool Trace share strict typed material, and descriptor-backed Tool Trace requests use strict canonical atom resolution. AgentCodex may change only the §11.2 allowlist plus its new implementation artifact; it must stop on any need for a compatibility path, new BusinessSource abstraction, Host import of Fins, or deferred Issue 177/178 behavior. R03 aggregate and later umbrella gates remain unauthorized. |
