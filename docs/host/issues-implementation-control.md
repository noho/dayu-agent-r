...
| draft PR status | WU-DUR-P01 / WU-OBS-P00 / WU-CM-01-F02 / WU-CM-01-F01 closeout PR 118 merged at 2026-06-05 13:17:00 UTC as d57c46b6fa43c2744ecbf45bdad68bdb3edf37c7; WU-CM-01 draft PR 116 was previously merged manually by the user; WU-ENG-02 PR 114 merged at 2026-06-03 09:33:38 UTC as 58fb7a42a2a096ab279863250a9ffe63f63f0edc; WU-ENG-01 accepted commit 70a5a4e merged via PR 113 |
| blocking open questions | none |
| current inspection note | 2026-06-05 PR 118 merge confirmed and branch `phaseflow/wu-tools-01` created from merge commit d57c46b6fa43c2744ecbf45bdad68bdb3edf37c7. WU-TOOLS-01 goal confirmation accepted by user. AgentCodex produced plan artifact `docs/host/wu-tools-01-migration-plan.md` and revised it after Controller/user constraints. AgentMiMo and AgentDS plan reviews completed. Controller accepted findings requiring plan fix: adapter API signatures, path metadata/enforcement boundary, current `ToolTruncateSpec` declaration mapping, input/response projection adapter contract, Fins ingestion blocker artifact destination, old sync callable concurrency boundary, S6 stop conditions, and old helper import-closure inventory. AgentCodex plan fix artifact `docs/reviews/wu-tools-01-plan-fix-codex.md` completed. AgentMiMo and AgentDS plan re-review artifacts both passed with no blocking findings; Controller accepted the plan in `docs/reviews/wu-tools-01-plan-rereview-controller-adjudication.md`. Accepted plan commit is f6658fb4. Slice S1 implementation completed; code review passed with findings; Controller accepted only the registry module-docstring wording fix and deferred coverage / OLD naming residuals. AgentCodex completed the narrow fix in `docs/reviews/wu-tools-01-slice1-fix-codex.md`; both re-reviews passed. Controller then found and dispatched a `tests/README.md` sync fix for the new `tests/documents/` layer; AgentCodex fixed it and both README sync re-reviews passed. Final validation passed: 18 targeted tests, 4 Engine import-boundary tests, pyright 0 errors, diff check clean. Slice S1 accepted commit is 07e3b7f9. Slice S2 implementation completed with target validation 89 passed, pyright 0 errors, diff check clean. Code reviews passed with accepted findings: batch `fetch_more` fail-fast consistency, provider-wide serialization / generic exception tests, strict OLD envelope detection, and path policy coverage fail-closed. AgentCodex fixed all accepted S2 findings; re-reviews passed and final validation evidence is 93 tests passed, pyright 0 errors, diff check clean. Slice S2 accepted commit is 20cd553c. Slice S3 implementation completed with target validation 19 passed, pyright 0 errors, diff check clean. Code reviews found stale OLD ToolRegistry owner wording, legacy register_allowed_paths path-safety code path, and list_files/search_files relative path chaining mismatch. AgentCodex fixed accepted findings; re-reviews passed. Final validation passed: 20 targeted tests, pyright 0 errors, staged diff check clean. Slice S3 accepted commit is 2d46fee8. Slice S4 implementation completed with validation 74 passed, pyright 0 errors, diff check clean. Ingestion tools were classified as requiring awaiting/wait-adapter semantics and documented in docs/reviews/wu-tools-01-s4-ingestion-blocker-codex.md. Code reviews accepted narrow type and test hardening findings; AgentCodex fixed them, including pyright warning follow-up. Re-reviews passed. Final validation passed: 75 targeted tests, pyright 0 errors/0 warnings, staged diff check clean. Slice S4 accepted commit is 6a0bc255. Slice S5 implementation migrated Web tools provider with deterministic Web tests; Controller found and accepted a blocking `Any` / `object` typing fix plus unused import, dead wrapper and Playwright cancellation projection fixes. Re-reviews passed. Final validation passed: 23 targeted tests, pyright 0 errors/0 warnings, `rg` only JSON schema `"object"` literals, staged diff check clean. Slice S5 accepted commit is 0b4dcd81. Next gate is Slice S6 implementation. |
...
| WU-TOOLS-01-R1 | WU-TOOLS-01 goal confirmation | path safety adapter | open | WU-TOOLS-01 implementation and post-migration design review | Accepted plan now requires Doc tools not to own path safety; `file_path_params` metadata is collected from migrated declarations and consumed by outer adapter / provider path validation. Path validation failure must return current `ToolFailedOutcome` before migrated Doc function bodies run. Keep tracking until implementation proves the boundary and post-migration review decides whether a better long-term design is needed. | Direct evidence: old `doc_tools.py` declares `file_path_params` and old `ToolRegistry` enforces `register_allowed_paths`; current `dayu-agent-r` `ToolDefinition` has schema / callable / truncate / display / tags but no registry-level file path metadata. |
| WU-TOOLS-01-R2 | WU-TOOLS-01 goal confirmation | typed config adapter | open | WU-TOOLS-01 implementation and post-migration design review | Accepted plan requires provider config to be parsed by outer provider / assembly code, with `dayu.runtime` staying layer-neutral. Track implementation of `DocToolLimits`, `FinsToolLimits`, and `WebToolsConfig` adaptation without hidden cwd/env fallback or Host imports of business implementations. | Direct evidence: old tool configs live in `dayu.contracts.tool_configs`; current `ConfigLoader` reads layer-neutral `tool_discovery.json` provider config and does not import Fins / Doc / Web implementation modules. |
| WU-TOOLS-01-R3 | WU-TOOLS-01 goal confirmation | ToolDiscovery / ToolRuntime adapter | open | WU-TOOLS-01 implementation and post-migration design review | Accepted plan defines a legacy declaration collector and adapter that convert migrated sync callables to current async `ToolCallable` and `ToolsDiscoveryProviderOutput`; OLD `ToolRegistry` remains out of scope. Track until ToolRuntime accept-path tests prove the adapter boundary and no forbidden Host / Engine imports exist. | Direct evidence: current `ToolsDiscovery` only resolves explicit provider callables / entry points; current `ToolCallable` is async single-tool protocol; ToolRuntime owns batch executor, truncation, duplicate governance and accept barrier. |
| WU-TOOLS-01-R4 | WU-TOOLS-01 user scope裁决 | truncation / fetch_more owner | open | WU-TOOLS-01 implementation and post-migration design review | Accepted plan prohibits migrating OLD `ToolRegistry`, OLD `TruncationManager`, OLD `fetch_more`, and OLD truncate / fetch-more projection. Migrated tools may only declare current `ToolTruncateSpec` metadata, while current ToolRuntime remains the truncation / framework `fetch_more` owner. Track tests that prove no OLD truncation runtime is imported or used. | Direct evidence: current `dayu-agent-r` ToolRuntime owns truncation and `FrameworkToolName.FETCH_MORE`; Host import-boundary tests require `fetch_more` stay inside ToolRuntime / tooling owner. |
| WU-TOOLS-01-R5 | WU-TOOLS-01 user scope裁决 | query / response projection adapter | open | WU-TOOLS-01 implementation and post-migration design review | Accepted plan defines input projection before calling migrated functions and response projection from migrated returns / OLD envelopes / exceptions to current outcomes. Track implementation so projection remains in the outer adapter and does not change migrated OLD function signatures or function bodies. | Direct evidence: current `ToolCallable` 接收 `ToolCallRequest.arguments` 与 `BatchToolExecutionContext`，返回 `ToolExecutionOutcome`；旧 registry tool 多为同步 Python 函数并返回 dict / 业务模型。 |
| WU-TOOLS-01-S3-R1 | WU-TOOLS-01 Slice S3 re-review | response projector placement | open | WU-TOOLS-01 later provider slices and post-migration design review | Slice S3 accepted a private `_legacy_adapter` response projection for Doc `list_files` / `search_files` reusable paths. This is acceptable while only one provider needs it, but if S4/S5 add more provider-specific response projection rules, revisit a declaration-level or provider-supplied projector instead of accumulating hard-coded tool-name branches in the generic adapter. | Direct evidence: `docs/reviews/wu-tools-01-slice3-rereview-controller-adjudication.md` accepted the current implementation as non-blocking and recorded the design-debt trigger for later slices. |
| WU-TOOLS-01-S4-R1 | WU-TOOLS-01 Slice S4 implementation | Fins ingestion waiting semantics | open | Later WU-TOOLS follow-up or Host ToolRuntime wait-adapter work unit | Fins ingestion start/status/cancel tools were not migrated because they return queued/running job snapshots and require durable waiting, polling, cancellation and late terminal result semantics. S4 provider keeps `include_ingestion_tools=true` fail-closed. | Direct evidence: `docs/reviews/wu-tools-01-s4-ingestion-blocker-codex.md` records affected tools, direct evidence from OLD ingestion job manager and required `ToolAwaitingOutcome` / wait-adapter semantics. |
| WU-TOOLS-01-S5-R1 | WU-TOOLS-01 Slice S5 implementation | Web provider concurrency policy | open | WU-TOOLS-01 Slice S6 and post-migration residual reconciliation | Web provider currently declares provider-level serial execution because shared requests session and Playwright fallback concurrency safety were not proven in S5. S6 combined ToolRuntime acceptance must verify this policy is honored or document any integration blocker. | Direct evidence: `docs/reviews/wu-tools-01-slice5-implementation-codex.md`, `docs/reviews/wu-tools-01-slice5-rereview-controller-adjudication.md`, and re-review artifacts record `SERIAL_PER_PROVIDER` as accepted residual. |
| WU-TOOLS-01-S5-R2 | WU-TOOLS-01 Slice S5 implementation | Web live network / real browser coverage | open | WU-TOOLS-01 Slice S6 and post-migration residual reconciliation | S5 intentionally used deterministic mocked search, requests and Playwright paths. No live network, real browser, Tavily or Serper API validation was performed. S6 must keep acceptance deterministic and decide whether this remains a deferred integration-test gap after migration. | Direct evidence: S5 implementation, code review and re-review artifacts all record no-live-network scope and deterministic mock validation. |
| WU-TOOLS-01-S1-R1 | WU-TOOLS-01 Slice S1 code review | documents coverage / parity | open | WU-TOOLS-01 later slices and final residual reconciliation | Later slices must cover `_doc_processor_factory.py`, `ProcessorRegistry` fallback paths, HTML pipeline primitives, and Docling runtime integration when those paths become consumed. S1 intentionally used lightweight deterministic Markdown, HTML and Docling JSON fixtures rather than full OLD parity or real PDF/OCR conversion. | Direct evidence: AgentDS coverage review reported low coverage for docling runtime, HTML pipeline modules, registry and factory. Accepted plan S1 only required lightweight deterministic fixtures; S3/S4/S5/S6 are the owners that consume those paths. |
| WU-TOOLS-01-S1-R2 | WU-TOOLS-01 Slice S1 code review | OLD processor registry naming | open | WU-TOOLS-01 post-migration design review | `build_engine_processor_registry(...)` remains intentionally unchanged in S1 to preserve migrated OLD function signatures. Revisit after WU-TOOLS-01 migration if public naming cleanup is still valuable and can be done as an explicit design cleanup. | Direct evidence: accepted plan S1 requires copied OLD processor class/function signatures and function bodies remain unchanged except import/package references; review finding identified the name as misleading but non-blocking. |
...
| WU-TOOLS-01 | implementation | Fins / Web / Doc tools migration with shared document foundations | GitHub Issue #82 / #97 / #98 | Accepted plan commit f6658fb4；Slices S1-S5 accepted through Web tools provider；下一入口为 S6 combined discovery / ToolRuntime acceptance / docs closure，且必须严格遵守迁移原则 |
