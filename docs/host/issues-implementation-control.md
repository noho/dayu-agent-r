...
| gate | R01 remediation sub-WU plan |
...
| next entry point | R01 internal remediation sub-WU plan gate. AgentCodex must plan Doc complete-input remediation from accepted umbrella plan commit `227317a0`, re-check the actual base and map every R01 umbrella baseline item to the R01 exact plan, preserve allowed-path/containment/symlink/cancellation/output-truncation/fetch-more behavior, remove only the controller-accepted input-size/directory-partial semantics, and stop before implementation. R01 is not a new WU and must complete its own dual plan review/fix/re-review/accepted-plan-commit sequence. |
