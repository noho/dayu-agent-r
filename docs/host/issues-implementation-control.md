...
| next entry point | Implementation gate for WU-TOOLS-CANCEL-01 residual hardening Slice S2A `Runtime Process Group Cleanup Primitive` on branch `phase/wu-tools-cancel-01`. Keep PR #170 draft until reopened gates pass and maintainer review. Do not mark ready, merge, close #87 directly, request reviewers, delete branch, or publish external closeout comments without explicit authorization. |
