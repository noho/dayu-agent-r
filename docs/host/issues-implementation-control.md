...
| implementation status | WU-TOOLS-01-F01-02 plan re-review passed; accepted plan commit in progress |
| active work unit | WU-TOOLS-01-F01-02 |
| default next work unit | WU-TOOLS-01-F01-02 |
| next entry point | WU-TOOLS-01-F01-02 accepted plan commit gate |
...
| WU-TOOLS-01-F01-02 | planning | Migrated tools cancellation propagation and response | WU-TOOLS-01-F01 draft PR preflight follow-up | Plan artifact `docs/host/wu-tools-01-f01-02-cancellation-plan.md`; plan review artifacts `docs/reviews/wu-tools-01-f01-02-plan-review-mimo.md`, `docs/reviews/wu-tools-01-f01-02-plan-review-ds.md`; controller adjudication `docs/reviews/wu-tools-01-f01-02-plan-review-controller-adjudication.md`; plan fix artifact `docs/reviews/wu-tools-01-f01-02-plan-fix-codex.md`; plan re-review artifacts `docs/reviews/wu-tools-01-f01-02-plan-rereview-mimo.md`, `docs/reviews/wu-tools-01-f01-02-plan-rereview-ds.md`; re-review controller adjudication `docs/reviews/wu-tools-01-f01-02-plan-rereview-controller-adjudication.md`; plan accepted and ready for accepted plan commit. 为当前已迁移 Fins / Web / Doc tools 补齐 `CancellationToken` 传递审计与取消响应；Host 仍是 cancel 治理真源；长事务工具必须在本 WU 实现取消响应；其它工具至少把 token 传入业务入口，并按阻塞 I/O、外部资源、CPU / 文件处理风险决定是否本 WU 补 checkpoint；不得用工具私有 cancel 状态替代 Host durable cancel；awaiting accept 前 orphan job 窗口需评估两阶段启动实现代价与 Host wait adapter / Fins runtime contract 扩展成本，未完成评估前不得默认扩 contract |
