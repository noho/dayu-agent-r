...
| next entry point | AgentCodex implements only R02-S1 from superseding accepted-plan commit `2d42ceb6`: packaged Web config plus typed HTTP/Browser/Diagnostic budget owners, immutable config snapshot and exact child projection, independent custom-port fact, transport-policy snapshot without S2 sender behavior, and the nine-file old-budget direct-consumer closure. The four drift-moved files remain limited to S1 owner type/signature/forwarding/direct-test changes; utility-local `1_024/default=80`, sender pinned/no-proxy, raw search requests, browser/private coupling, storage-state lifecycle and all S2/S3 behavior remain unchanged. AgentCodex must produce the fixed S1 implementation artifact, affected tests, per-file coverage, full pyright, README decision, source scans and `git diff --check`, then stop for dual code review. Any fifth drift file, non-allowlist product/test/README change, deferred Issue, R03 or unified authorization scope requires Controller stop/adjudication. |
