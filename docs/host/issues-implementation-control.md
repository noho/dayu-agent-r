...
| gate | R03 accepted local commit |
...
| next entry point | Complete R03 aggregate re-review artifacts are `docs/reviews/wu-semantic-ownership-01-r03-aggregate-rereview-mimo.md` and `docs/reviews/wu-semantic-ownership-01-r03-aggregate-rereview-ds.md`; both return `PASS / 0 accepted findings / 0 blocking questions` and keep F01-F03 `CLOSED`. Controller adjudication is `docs/reviews/wu-semantic-ownership-01-r03-aggregate-rereview-controller-adjudication.md` with decision `ACCEPTED_AGGREGATE_RE_REVIEW / ZERO_FINDING / ACCEPTED_LOCAL_COMMIT_AUTHORIZED`. Only the exact R03 aggregate product/test/smoke/artifact/control scope listed in that adjudication may be staged and committed. Before commit, Controller must rerun `git diff --check`, verify exact staged paths and no credential/raw config or deferred owner path. No push or R04 content is authorized in that commit. |
