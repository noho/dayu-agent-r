...
| gate | accepted-slice |
| implementation status | WU-CM-13 Slice 2a implementation and code review PASS. Proactive dispatch now uses shared compact pipeline helpers for normal request, tier 1-3 recovery requests, and fallback decision input while keeping lifecycle, EventLog append, artifact write, fail-unstarted, stale guards, cursor guards, and dispatch start in `dispatch.py`. |
| active work unit | WU-CM-13 |
| default next work unit | WU-CM-13 |
| next entry point | Create WU-CM-13 Slice 2a accepted slice commit, then dispatch Slice 2b reactive ingest wiring using accepted plan `docs/host/host-issues/wu-cm-13-unified-compact-pipeline-plan.md` and Slice 2a review adjudication `docs/reviews/code-review-wu-cm-13-slice-2a-adjudication-20260619.md`. |
...
| blocking open questions | None for WU-CM-13 Slice 2b after Slice 2a accepted commit. WU-CLI-DEBUG-STREAM-01 still needs discussion / plan confirmation before code changes. Draft PR #150 remains open draft; merge, mark-ready, reviewer requests, and issue closure require separate user authorization. |
...
| WU-CM-14-RR-1 | deferred-with-owner | WU-CM-13 Unified conversation compact pipeline convergence | WU-CM-14 fixed protected recent raw tail preservation, but full reactive "freeze exact overflow ordinary material list" convergence remains broader than WU-CM-14. WU-CM-13 must re-audit WU-CM-14 preservation in the shared compact pipeline and decide whether to replace EventLog re-read handoff with a unified material view owner. |
| WU-CM-14-RR-3 | deferred-with-owner | WU-CM-13 Unified conversation compact pipeline convergence | WU-CM-14 deliberately accepts a second EventLog read inside the protected recent raw tail provider. WU-CM-13 must revisit whether the unified compact pipeline should pass one frozen `PreDispatchCompactMaterialView` / material handoff into both compact and RunInput assembly. |
| WU-CM-13-S1-R1 | deferred-with-owner | WU-CM-13 Slice 2 wiring / compact quality gate audit | Slice 1 review noted that old malformed compacted payload fact-ref tests do not map one-to-one to the new typed helper boundary. Slice 2 must either add a focused evidence-label / provenance validation test at the accepted compact quality gate or document existing operation-level coverage that closes the edge. |
| WU-CM-13-S1-R2 | deferred-with-owner | WU-CM-13 Slice 2 proactive / reactive wiring | Slice 1 intentionally leaves tiny `_dedupe_texts`-style helpers duplicated in `dispatch.py` / `engine_ingest.py`. Slice 2a / 2b must remove the duplicate by switching those paths to `compact_pipeline.py`, or explicitly justify any remaining duplication. |
...
| WU-CM-13 | accepted-slice | Unified conversation compact pipeline convergence | WU-CM-12-S4-R1 follow-up；无 GitHub Issue | Slice 2a implementation and code review PASS. Proactive dispatch now uses shared compact pipeline helpers; next gate is Slice 2a accepted commit followed by Slice 2b reactive ingest wiring. |
...
- next gate: accepted Slice 2a commit, then Slice 2b implementation.
