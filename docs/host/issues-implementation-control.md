...
| next entry point | Implementation gate for WU-TOOLS-CANCEL-01 S2B `Doc process-backed` on branch `phase/wu-tools-cancel-01`. Migrate Doc production blocking tool execution to `process_backed` through `ToolDefinition.execution`; process targets must be serializable, recreate document processors in the child process, preserve path containment / output shape / truncation semantics, and keep cancel / timeout owned by the parent Host capsule. Stop if Doc process-backed requires parent-process provider lock, Host internals in business tools, Engine contract changes, or changing `dayu.runtime.interruptible_process` away from `JsonValue`. |
...
| blocking open questions | None for S2B entry. User confirmed WU-TOOLS-CANCEL-01 goal: #87 closeout requires Codex / Claude Code style Esc interrupt user experience, with WU-LIFE-03 / WU-LIFE-04 / WU-WAIT-03 consumed as completed prerequisites. |
