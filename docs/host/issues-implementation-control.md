...
| next entry point | Implementation gate for WU-TOOLS-CANCEL-01 residual hardening Slice S2B `Playwright Cleanup Smoke` on branch `phase/wu-tools-cancel-01`. Keep PR #170 draft until reopened gates pass and maintainer review. Do not mark ready, merge, close #87 directly, request reviewers, delete branch, or publish external closeout comments without explicit authorization. |
...
| blocking open questions | None for Slice S2B implementation. User confirmed Web process cold-start may remain deferred if it only affects performance and not cancellation robustness. Plan re-review accepted one deferred-with-owner item for future Web cold-start / performance hardening if S2B evidence shows cancellation robustness impact. S2B must call `enter_new_process_session_if_supported()` or equivalent POSIX setup inside the Web worker child before spawning nested browser processes before claiming nested cleanup. |
