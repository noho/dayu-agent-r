...
| gate | R01 aggregate accepted local commit |
...
| next entry point | Controller creates the authorized R01 aggregate accepted local commit containing aggregate validation, both PASS aggregate deepreviews, aggregate controller adjudication, completion artifact, completion controller validation and current control state. This commit only makes internal remediation sub-WU R01 locally accepted; it does not close the umbrella WU, implement Issue 177, authorize a unified tool authorization framework, or enter R03. After commit, controller must separately record its hash and enter R02's independent plan gate from the current code/evidence baseline. |
