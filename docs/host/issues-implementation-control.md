...
| next entry point | Design/contract gate for WU-TOOLS-CANCEL-01 typed tool execution capability on branch `phase/wu-tools-cancel-01`. Define how production ToolRuntime selects `async_direct`, `thread_backed`, or `process_backed` from typed tool declarations without Host tool-name branching and without business tools importing Host internals; then resume Slice S2 migration for Doc, Fins read, and Web sync paths. |
...
| blocking open questions | WU-TOOLS-CANCEL-01 has an active design/contract question: production tools need a typed execution capability declaration so ToolRuntime can choose process-backed or request-abort-capable execution without violating layering. User confirmed WU-TOOLS-CANCEL-01 goal: #87 closeout requires Codex / Claude Code style Esc interrupt user experience, with WU-LIFE-03 / WU-LIFE-04 / WU-WAIT-03 consumed as completed prerequisites. |
...
| WU-TOOLS-CANCEL-01 | design/contract | Tool/provider blocking I/O cancellation hardening | follows WU-LIFE-04 | WU-LIFE-04 已完成；accepted plan commit 为 `4723ec61`，S1 accepted slice commit 为 `eda4be1a`。S2 implementation attempt 保留 Web timeout-budget propagation 与 Playwright unpicklable worker fail-closed 作为 partial hardening；AgentMiMo / AgentDS review 与 controller adjudication 均确认 Doc / Fins read / Web sync 生产路径因缺少 typed execution capability 声明而命中全局 stop condition，不能标记 S2 完成或关闭 #87。当前下一步是 design/contract gate：定义 ToolRuntime 如何从 typed tool declarations 选择 `async_direct`、`thread_backed`、`process_backed`，不得使用 Host tool-name branch，不得让业务工具 import Host internals。生产级目标仍是 Codex / Claude Code 类似 interrupt 体感：取消后 Host 迅速回到可交互状态，旧结果不能污染已取消 Run，且不得延长单次工具执行 deadline；不可抢占 blocking I/O 必须迁移到可中断 capsule 或被禁止进入 production cancel path。 |
