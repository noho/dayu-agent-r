...
| gate | R01 remediation sub-WU accepted-plan local commit |
...
| next entry point | Controller must create the R01 accepted-plan local commit containing the final plan and complete review/fix/re-review/adjudication chain, then record its SHA and enter only `R01-S1` implementation. R01-S2 and all later remediation sub-WUs remain unauthorized until their own gates. |
