...
| gate | review |
| implementation status | residual-risk-rereview-pass |
| active work unit | WU-ENG-02 |
| default next work unit | WU-CM-01 |
| next entry point | commit residual risk fix for PR 114 |
...
| draft PR status | WU-ENG-02 draft PR https://github.com/noho/dayu-agent-r/pull/114 reopened for residual-risk review; WU-ENG-01 PR 113 merged at 2026-06-03 05:14:07 UTC as bc50e26c45296171487272ff5fc2293db67a9246 |
...
|---|---|---|---|---|---|---|
| WU-ENG-02-S1-R1 | WU-ENG-02 Slice 1 re-review | 测试 / 诊断一致性 | closed | WU-ENG-02 residual risk fix gate | 无后续动作 | PR 114 residual fix 关闭：工具超时 terminal `RunFailedData` 显式写入触发当前工具批的 `decision.client_correlation_id`，并由 Engine focused tests 直接断言。 |
| WU-ENG-02-S1-R2 | WU-ENG-02 Slice 1 re-review | 测试覆盖 | closed | WU-ENG-02 residual risk fix gate | 无后续动作 | PR 114 residual fix 关闭：force-answer 空内容与继续 tool call failure 的 emitted `RunFailedData.client_correlation_id` 均直接断言为第二次 Runner request identity。 |
...
| WU-ENG-02-S2-R2 | WU-ENG-02 Slice 2 code review | 错误收口 | closed | WU-ENG-02 residual risk fix gate | 无后续动作 | PR 114 residual fix 关闭：静态 `X-Client-Request-Id` 与 `OPENAI_X_CLIENT_REQUEST_ID` policy 的冲突已前移到 `RunnerSpec` construction boundary，OpenAI runner 只负责 header 映射。 |
| WU-ENG-02-S3-R1 | WU-ENG-02 Slice 3 code review | analyzer 需求边界 | deferred-with-owner | WU-OBS-00 / GitHub Issue #70 analyzer | analyzer 实施时确认 usage observation projection signal 是否需要 `client_correlation_id` 与 `provider_request_id`，若需要先扩展 Engine `UsageReportedData` / Runner usage event contract，再补 Host payload / Tool Trace tests | PR 114 residual review 裁决保留 defer：usage 是 post-call observation / analyzer signal，不是 provider debugging terminal 主链路；`UsageReportedData` 当前无 correlation fields，且 usage event 发生时 provider request id 不总是可用，不能在 residual fix gate 单独定死 analyzer 关联语义。 |
| WU-ENG-02-S3-R2 | WU-ENG-02 Slice 3 code review | 测试覆盖 | closed | WU-ENG-02 residual risk fix gate | 无后续动作 | PR 114 residual fix 关闭：Host durable focused tests 直接断言 context recovery closeout 的 ATTEMPT_FAILED / RUN_RECOVERING payload 保留 `client_correlation_id`，并断言空白值被拒绝。 |
