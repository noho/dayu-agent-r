...
| next entry point | WU-LIFE-03 / GitHub Issue #91 accepted Slice 2 commit, then aggregate deepreview for the completed WU-LIFE-03 local changes. |
...
| WU-LIFE-03 | accepted-slice | Active cancel watchdog | GitHub Issue #91 / #87 umbrella | Slice 2 code re-review 已通过，等待 accepted Slice 2 commit；之后进入 aggregate deepreview。固定 Host-level active cancel watchdog、post-cancel timeout closeout、late terminal race 和 diagnostic 语义。只负责 Host truth / timeout closeout，不负责 tool/provider hard interrupt。 |
