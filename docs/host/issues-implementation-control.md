...
| next entry point | Collect user manual validation evidence for MANUAL-01 and MANUAL-02; MANUAL-03 remains optional unless the user widens the WU. |
...
| blocking open questions | Manual evidence is required before final closeout: MANUAL-01 real provider `dayu-cli prompt`; MANUAL-02 real provider `dayu-cli interactive` running-state Ctrl+C. MANUAL-03 real Fins direct download / process remains optional unless the user widens scope. |
