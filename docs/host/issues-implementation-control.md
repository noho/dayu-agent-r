...
| Fins cancel / memory continuity follow-up | User-reported `dayu-cli interactive` cancel/retry failure after `下载Circle财报` is closed locally. Root-cause artifact: `docs/reviews/wu-cli-smoke-01-fins-cancel-memory-root-cause-codex.md`; implementation/fix artifact: `docs/reviews/wu-cli-smoke-01-fins-cancel-memory-review-fix-codex.md`; review artifacts: `docs/reviews/wu-cli-smoke-01-fins-cancel-memory-review-mimo.md` and `docs/reviews/wu-cli-smoke-01-fins-cancel-memory-review-ds.md`; re-review artifacts: `docs/reviews/wu-cli-smoke-01-fins-cancel-memory-rereview-mimo.md` and `docs/reviews/wu-cli-smoke-01-fins-cancel-memory-rereview-ds.md`. Direct evidence showed Host durable truth had cancelled the first run and abandoned the wait, while the Fins observation still continued cooperatively, SEC reused an owned `httpx.AsyncClient` across closed `asyncio.run(...)` event loops causing the second run `Event loop is closed`, and memory did not project first-run `TOOL_AWAITING.accepted_arguments={"ticker":"CRCL"}` as LLM-facing continuity. The fix keeps Fins download/preprocess/upload on the designed awaiting poll path, adds producer and SEC/CN/HK single-filing cancellation checkpoints, refreshes SEC owned HTTP clients by actual event-loop object identity, preserves CN/HK cancellation exception identity, and projects verified LLM-safe awaiting accepted arguments into conversation memory without leaking wait ids, event ids, digests, or scheduler state. Controller validation passed affected tests (`318 passed`, existing edgar deprecation warnings), pyright (`0 errors`), and `git diff --check`; AgentMiMo / AgentDS re-review passed with accepted findings closed. Residual risk: CN/HK Docling synchronous third-party conversion cannot be hard-interrupted while running inside `asyncio.to_thread(...)`; if product requires hard timeout during conversion, a later WU should move that conversion boundary to process/subprocess isolation with timeout. |
