...
| current follow-up status | Context slot / FMP / scene tool filtering follow-up: accepted plan commit `645c7473`; Slice S1 scene filtering accepted commit `2824ee59`; Slice S2 FMP resolver / entrypoint context slots accepted commit `b521ff39`; Slice S3 prompt assets / manifests / docs accepted commit `69ad3a85`; aggregate real-environment prompt smoke accepted commit `c4ca4d55`; user-identified `{{fins_default_subject}}` placement fix accepted commit `0a029d88`. Placement fix artifacts are `docs/reviews/wu-cli-smoke-01-context-slot-placement-fix-codex.md`, `docs/reviews/wu-cli-smoke-01-context-slot-placement-review-mimo.md`, `docs/reviews/wu-cli-smoke-01-context-slot-placement-review-ds.md`, `docs/reviews/wu-cli-smoke-01-context-slot-placement-rereview-mimo.md`, `docs/reviews/wu-cli-smoke-01-context-slot-placement-rereview-ds.md`, and `docs/reviews/wu-cli-smoke-01-context-slot-placement-controller-adjudication.md`. Real `dayu-cli prompt` validation used provider `mimo` / `mimo-v2.5-pro` with HTTP 200: `--ticker V` produced `V（Visa Inc.）` and called `get_current_time`; no-ticker prompt did not expose “未指定具体公司” and made no tool call; debug logs and Host sqlite payload checks found no `base_user` or long-transaction tool names. Controller validation also passed placement runtime scene tests `51 passed`, prompt path / CLI prompt tests `41 passed`, FMP resolver tests `8 passed`, Service tests `102 passed`, CLI tests `91 passed`, smoke assembly tests `7 passed`, source scan with no `base_user` / `BASE_USER` source matches, pyright `0 errors`, and `git diff --check`. |
