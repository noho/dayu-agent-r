...
| gate | PR review |
| implementation status | draft-pr-open |
...
| implementation commits | accepted plan commit 59f66b7; Slice 1 accepted commit c4826e0; Slice 2 accepted commit c3856b9; Slice 3 accepted commit 5ddc4cb; Slice 4 accepted commit 896d483; accepted deepreview commit 24af62b; WU-ENG-01 accepted commit 70a5a4e merged via PR 113 |
| review artifacts | docs/reviews/wu-eng-02-plan-review-mimo.md; docs/reviews/wu-eng-02-plan-review-ds.md; docs/reviews/wu-eng-02-plan-fix-codex.md; docs/reviews/wu-eng-02-plan-rereview-mimo.md; docs/reviews/wu-eng-02-plan-rereview-ds.md; docs/reviews/wu-eng-02-slice1-implementation-codex.md; docs/reviews/wu-eng-02-slice1-code-review-mimo.md; docs/reviews/wu-eng-02-slice1-code-review-ds.md; docs/reviews/wu-eng-02-slice1-fix-codex.md; docs/reviews/wu-eng-02-slice1-rereview-mimo.md; docs/reviews/wu-eng-02-slice1-rereview-ds.md; docs/reviews/wu-eng-02-slice2-implementation-codex.md; docs/reviews/wu-eng-02-slice2-code-review-mimo.md; docs/reviews/wu-eng-02-slice2-code-review-ds.md; docs/reviews/wu-eng-02-slice2-fix-codex.md; docs/reviews/wu-eng-02-slice2-rereview-mimo.md; docs/reviews/wu-eng-02-slice2-rereview-ds.md; docs/reviews/wu-eng-02-slice3-implementation-codex.md; docs/reviews/wu-eng-02-slice3-code-review-mimo.md; docs/reviews/wu-eng-02-slice3-code-review-ds.md; docs/reviews/wu-eng-02-slice4-implementation-codex.md; docs/reviews/wu-eng-02-slice4-code-review-mimo.md; docs/reviews/wu-eng-02-slice4-code-review-ds.md |
...
| draft PR status | WU-ENG-02 draft PR https://github.com/noho/dayu-agent-r/pull/114 open; WU-ENG-01 PR 113 merged at 2026-06-03 05:14:07 UTC as bc50e26c45296171487272ff5fc2293db67a9246 |
...
|---|---|---|---|---|---|---|
| WU-ENG-02-S1-R1 | WU-ENG-02 Slice 1 re-review | 测试 / 诊断一致性 | deferred-with-owner | aggregate review / future Engine focused test | aggregate review 裁决工具超时类 `RunFailedData` 是否需要携带 current call correlation；若需要，补 Engine path 与 Host ingest 测试 | `_ERROR_TOOL_EXECUTION_TIMEOUT` 路径目前仍未携带 `client_correlation_id`；Slice 4 复核认为该路径发生在工具执行等待阶段，current Runner call correlation 语义需另行裁决。 |
| WU-ENG-02-S1-R2 | WU-ENG-02 Slice 1 re-review | 测试覆盖 | deferred-with-owner | aggregate review / future Engine focused test | aggregate review 裁决 force-answer failure EngineEvent 级 `client_correlation_id` 直接断言是否必须补齐；若必须，补 focused Engine test | force-answer 代码行为已携带 `_client_correlation_id_from_state(state)`，现有测试只间接覆盖 runner identity / call index，缺少 emitted EngineEvent 直接断言。 |
| WU-ENG-02-S2-R1 | WU-ENG-02 Slice 2 code review | 配置 / 集成决策 | closed | WU-ENG-02 Slice 4 final validation | 无后续动作 | Slice 4 复核关闭：`ClientCorrelationPolicy.DISABLED` 是显式 policy，OpenAI Runner tests 覆盖 disabled 不发送 header，Host effective config tests 覆盖 enabled freeze / restore；production assembly 默认 disabled 符合“显式 policy 才发送”验收。 |
| WU-ENG-02-S2-R2 | WU-ENG-02 Slice 2 code review | 错误收口 | deferred-with-owner | Service / config assembly follow-up | Engine adapter 部分已关闭；若需要 Service-facing 结构化配置错误收口，由 Service/config assembly follow-up 承接 | OpenAI Runner 已在 HTTP post 前 fail fast 并由测试确认不发请求；上层 `ValueError` 到 Service-facing structured error 的收口不属于本 WU 最小验收。 |
| WU-ENG-02-S3-R1 | WU-ENG-02 Slice 3 code review | analyzer 需求边界 | deferred-with-owner | WU-OBS-00 / GitHub Issue #70 analyzer | analyzer 实施时确认 usage observation projection signal 是否也需要 `client_correlation_id`；若需要，先扩展 Engine `UsageReportedData` 或等价 contract，再补 Host payload / Tool Trace tests | Slice 3 只覆盖 provider-related EngineEvent payload；`UsageReportedData` 当前没有 `client_correlation_id`，其 payload 里的 `provider_request_id` 也是 hardcoded `None`，不阻塞 provider debugging correlation 主链路。 |
| WU-ENG-02-S3-R2 | WU-ENG-02 Slice 3 code review | 测试覆盖 | deferred-with-owner | aggregate review / future Host focused test | aggregate review 裁决是否需要为 `ContextRecoveryCloseInput.client_correlation_id` 增加专用 validation / payload 单测；若必须，补 focused Host test | Slice 4 复核认为当前不阻塞：字段与 `provider_request_id` 校验完全对称，reactive compaction / context recovery 路径已有间接覆盖。 |
