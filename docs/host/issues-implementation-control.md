...
| implementation status | WU-TOOLS-01 S1-S6 and external blocker reconciliation accepted; Host compactor seam blocker deferred-with-owner |
| active work unit | WU-TOOLS-01 |
| default next work unit | WU-TOOLS-01 |
| next entry point | User decision: open WU-TOOLS-01 draft PR with deferred Host compactor seam blocker, or start Host compactor seam follow-up first |
...
| wu-tools slice6 artifacts | docs/reviews/wu-tools-01-slice6-implementation-codex.md; docs/reviews/wu-tools-01-slice6-code-review-mimo.md; docs/reviews/wu-tools-01-slice6-code-review-ds.md; docs/reviews/wu-tools-01-slice6-code-review-controller-adjudication.md; docs/reviews/wu-tools-01-slice6-fix-codex.md; docs/reviews/wu-tools-01-slice6-rereview-mimo.md; docs/reviews/wu-tools-01-slice6-rereview-ds.md; docs/reviews/wu-tools-01-slice6-rereview-controller-adjudication.md |
| wu-tools external blocker reconciliation artifacts | docs/reviews/wu-tools-01-external-blocker-reconciliation-codex.md; docs/reviews/wu-tools-01-external-blocker-reconciliation-review-mimo.md; docs/reviews/wu-tools-01-external-blocker-reconciliation-review-ds.md; docs/reviews/wu-tools-01-external-blocker-reconciliation-controller-adjudication.md |
...
| WU-TOOLS-01-S6-R1 | WU-TOOLS-01 Slice S6 broad validation | WU-CM-01 compact test seam fallout | deferred-with-owner | Host compactor seam / WU-CM-01 follow-up before broad green validation | Broad validation still has 7 Host proactive compaction failures with `accepted compaction is missing proposal manifest ref`. This is not a split between ordinary generic compactor and a prepared fake compactor. Root cause is that WU-CM-01 upgraded ConversationMemory / Compact to require accepted compact outcomes to reverse-reference durable proposal manifest ref / digest, while the proactive scheduler tests still use a legacy fake compaction seam from the pre-upgrade compact contract. Fix plan: keep production fail-closed behavior in `dayu/host/dispatch.py` unchanged; add or extract a deterministic manifest-producing Host test compactor aligned with `CompactorProposalPreparedCompactor`; migrate the affected proactive scheduler tests to that seam; assert accepted and rejected compact events carry the expected proposal manifest ref / digest; then rerun the broad Host validation. | Direct evidence: the 7 failures all raise `accepted compaction is missing proposal manifest ref`; current production contract is expressed by `CompactorProposalPreparedCompactor` and the accepted closeout guard. Existing focused tests already demonstrate the target seam shape in `tests/host/test_compaction_operation.py` and `tests/host/test_engine_ingest_mapping.py`; this is a Host compactor / WU-CM-01 follow-up, not a WU-TOOLS provider migration fix. |
