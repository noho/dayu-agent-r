...
| `CONTEXT_BUDGET_EVALUATED` | `session_id`、`run_id`、input snapshot cursor / digest | sizing stage（`ordinary` / `post_compact` / `reactive_post_compact` / `dispatch_fallback`）、sizing method、predicted input tokens、context window、soft / hard threshold tokens、pressure level、anchor / estimator diagnostic refs | 无 Run / Attempt 状态副作用；记录并公开本次 ordinary / proactive post-compact / reactive accepted post-compact / dispatch-fallback 候选输入的 dispatch-relevant sizing result；必须先于其驱动的 compact request 或 dispatch lifecycle facts 提交 | resume 不进入 messages；recovery 只消费 digest-verified internal refs 重建 decision | audit / trace 是；Host event stream 投影安全 typed context-usage activity |
...
- Host 内部按 ratio 计算 soft / hard threshold tokens。`pressure_level` 只表达 predicted tokens 与 thresholds 的比较结果，不因执行阶段改写：达到 hard 为 `hard_threshold_exceeded`，否则达到 soft 为 `soft_threshold_exceeded`，否则为 `normal`。实际 action 还必须消费 sizing stage：
  ...
  post-compact / fallback / reactive post-compact 的soft pressure不得伪装成normal；`reactive_post_compact` hard也必须如实保留hard pressure与allow action。reactive accepted compact后的estimate不是lifecycle failure真源，不得因此追加`CONTEXT_COMPACTION_FAILED`、`RUN_FAILED`或`RUN_LOST`。对同一snapshot已启动proactive operation后，任何post-compact / fallback sizing都不得启动第二次proactive operation；`reactive_post_compact`也不得启动proactive operation。
