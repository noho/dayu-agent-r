...
当前 gate 追加事实（Phase 12.2 accepted local commit）：Accepted Phase 12.2 implementation commit hash 为 `f570b26`；本条总控 bookkeeping 记录由后续文档提交承载。implementation commit 未包含用户要求保持不动的两个 pre-existing full-repo review artifacts：`docs/reviews/repo-review-20260522-070034.md` 与 `docs/reviews/repo-review-20260522-070045.md`。当前 gate 为 accepted-local-PASS。

当前 gate 追加事实（full-repo review adjudication）：两份并行 full-repo review artifacts 为 `docs/reviews/repo-review-20260522-070034.md` 与 `docs/reviews/repo-review-20260522-070045.md`。Controller adjudication artifact 为 `docs/reviews/repo-review-20260522-controller-adjudication.md`。总控裁决：接受 A1-A6 为当前安全 fix，覆盖 `ToolTruncateSpec` empty limits 测试同步、`fetch_more` ownership allowlist、disabled truncate spec target / TTL fail-fast、`_PublicHostHandle._closed` bool 注解、`MergedAgentPolicyConfig.field_sources` runtime immutable、ConfigLoader catalog 非空 guards。Engine findings、durable split、`merge_agent_policy_config` 大重构、`LaneController.acquire` 重构、`dayu/host/api.py` 拆分、README dead-link cleanup 等均 deferred 到后续专门 gate。当前进入 full-repo review accepted fix。
...
当前 gate 追加事实（Phase 12.3 Slice 1 accepted）：Accepted Slice 1 local commit hash 为 `6cde5ca`。当前进入 Phase 12.3 Slice 2 implementation。

当前 gate 追加事实（Phase 12.3 Slice 2 implementation）：Phase 12.3 Slice 2 implementation artifact 为 `docs/reviews/phase12-3-slice2-implementation-codex-20260522.md`；implementation agent 在 Host Context Governance / Engine ingest 中增加 usage observation diagnostic helper 与 `USAGE_REPORTED` projection signal 关联字段，保持 Engine usage contracts 不变，usage 缺失、估算缺失或异常 usage 均降级为 diagnostic，不改变 Run / Attempt 状态。AgentCodex reported：`pytest tests/host/test_engine_ingest_mapping.py tests/host/test_context_budget.py -q` 62 passed；`pytest tests/engine/runners/openai/test_stream_usage_capability_gating.py tests/engine/runners/openai/test_non_stream_response.py tests/engine/runners/openai/test_sse_usage_recorded.py -q` 11 passed；`python -m pyright dayu/host tests/host tests/engine/runners/openai` 0 errors；`git diff --check` clean。当前进入 Phase 12.3 Slice 2 code review。

当前 gate 追加事实（Phase 12.3 Slice 2 code review）：Phase 12.3 Slice 2 code review artifacts 为 `docs/reviews/phase12-3-slice2-code-review-mimo-20260522.md` 与 `docs/reviews/phase12-3-slice2-code-review-ds-20260522.md`，两份均 PASS，blocking finding count = 0。Controller adjudication artifact 为 `docs/reviews/phase12-3-slice2-code-review-controller-adjudication-20260522.md`；总控裁决：接受 DS advisory findings P12.3-S2-F1 / F2 / F3 为当前窄 fix，分别收窄异常边界、将 `iteration_id` 纳入 usage observation digest、修正 `_display_text_from_input_event` docstring。当前进入 Phase 12.3 Slice 2 fix。

当前 gate 追加事实（Phase 12.3 Slice 2 fix）：P12.3-S2-F1 / F2 / F3 已修复，fix addendum 已追加到 `docs/reviews/phase12-3-slice2-implementation-codex-20260522.md`；`_estimate_usage_observation_input` 只捕获 `HostDurableError` / `TypeError` / `ValueError`，`UsageObservation` 与 digest 纳入 `iteration_id`，`_display_text_from_input_event` docstring 与实际异常一致。AgentCodex reported：`pytest tests/host/test_engine_ingest_mapping.py tests/host/test_context_budget.py -q` 62 passed；`pytest tests/engine/runners/openai/test_stream_usage_capability_gating.py tests/engine/runners/openai/test_non_stream_response.py tests/engine/runners/openai/test_sse_usage_recorded.py -q` 11 passed；`python -m pyright dayu/host tests/host tests/engine/runners/openai` 0 errors；`git diff --check` clean。当前进入 Phase 12.3 Slice 2 re-review。

当前 gate 追加事实（Phase 12.3 Slice 2 re-review accepted）：Phase 12.3 Slice 2 re-review artifacts 为 `docs/reviews/phase12-3-slice2-rereview-mimo-20260522.md` 与 `docs/reviews/phase12-3-slice2-rereview-ds-20260522.md`，两份均 PASS，确认 P12.3-S2-F1 / F2 / F3 已收口且无新增 blocker。Controller re-review adjudication artifact 为 `docs/reviews/phase12-3-slice2-rereview-controller-adjudication-20260522.md`；总控裁决：接受 Slice 2。当前进入 Phase 12.3 Slice 2 accepted local commit。

当前 gate 追加事实（Phase 12.3 Slice 2 accepted）：Accepted Slice 2 local commit hash 为 `bf308ec`。当前进入 Phase 12.3 Slice 3 implementation。

当前 gate 追加事实（Phase 12.3 Slice 3 implementation）：Phase 12.3 Slice 3 implementation artifact 为 `docs/reviews/phase12-3-slice3-implementation-codex-20260522.md`；implementation agent 将默认 execution profile 改为 `standard-256k`，新增 `standard-1m`、`wechat-256k`、`wechat-1m`，ConfigLoader 增加 `context_window_class` / `min_context_window_tokens` schema，runtime assembly 增加 profile / model compatibility diagnostic helper，Service assembly 记录 ordinary / compactor compatibility diagnostics 且不自动切换 profile。AgentCodex reported：`pytest tests/runtime/test_config_loader.py tests/runtime/test_assembly_helpers.py tests/service/test_host_assembly.py -q` 51 passed；`pytest tests/runtime/test_import_boundary.py tests/runtime/test_weak_typing_guard.py -q` 13 passed；`python -m pyright dayu/runtime dayu/service tests/runtime tests/service` 0 errors；`git diff --check` clean。当前进入 Phase 12.3 Slice 3 code review。

当前 gate 追加事实（Phase 12.3 Slice 3 code review）：Phase 12.3 Slice 3 code review artifacts 为 `docs/reviews/phase12-3-slice3-code-review-mimo-20260522.md` 与 `docs/reviews/phase12-3-slice3-code-review-ds-20260522.md`。MiMo reported blocking finding：`tests/runtime/test_smoke_host_public_multiturn_assembly.py` 旧 `standard` profile id 残留导致 smoke assembly test 失败；DS PASS 但提出 `dayu.runtime.assembly.__all__` 缺新 helper 符号、`context_window_class` 与 `min_context_window_tokens` 缺交叉校验。Controller adjudication artifact 为 `docs/reviews/phase12-3-slice3-code-review-controller-adjudication-20260522.md`；总控裁决：三项均接受为当前窄 fix。当前进入 Phase 12.3 Slice 3 fix。

当前 gate 追加事实（Phase 12.3 Slice 3 fix）：P12.3-S3-F1 / F2 / F3 已修复，fix addendum 已追加到 `docs/reviews/phase12-3-slice3-implementation-codex-20260522.md`；smoke assembly test 已迁移为 `standard-256k`，新 runtime compatibility helper 符号已加入 `__all__`，ConfigLoader 已交叉校验 `256k -> 262144` 与 `1m -> 1000000`。AgentCodex reported：`pytest tests/runtime/test_config_loader.py tests/runtime/test_assembly_helpers.py tests/service/test_host_assembly.py tests/runtime/test_smoke_host_public_multiturn_assembly.py -q` 56 passed；`pytest tests/runtime/test_import_boundary.py tests/runtime/test_weak_typing_guard.py -q` 13 passed；`python -m pyright dayu/runtime dayu/service tests/runtime tests/service` 0 errors；`git diff --check` clean。当前进入 Phase 12.3 Slice 3 re-review。
...
当前 gate 追加事实（PR 67 draft-PR-pass）：PR 67 title 已更新为 `Phase 12 runtime assembly and config governance`。PR 67 post-push review record commit 为 `ad2f205`，已推送到 `github/docs/phase12-design-discussion`；最终控制记录也已随分支推送。`gh pr view 67` 在 accepted review record 推送后显示 draft=true、state=OPEN、mergeStateStatus=CLEAN、statusCheckRollup=[]；`gh pr checks 67 --watch=false` 返回 no checks reported。PR 67 draft PR gate 通过，当前状态为 `draft-PR-pass`。后续 merge、mark ready for review、request reviewers、delete branch、对外 comment 或外部 issue 仍需用户额外授权。
