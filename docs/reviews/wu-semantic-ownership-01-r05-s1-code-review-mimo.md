# WU-SEMANTIC-OWNERSHIP-01 R05-S1 Adversarial Code Review — AgentMiMo (第一路)

## 1. Review scope 与 verdict

- review gate：`R05-S1 dual complete code review`；第一路 AgentMiMo adversarial review。
...
经逐行 adversarial review 七路径 product/test/design diff 全文、R05 plan、implementation artifact、validation continuation artifact、Controller validation、no-diff owner source scan、focused/functional/pyright/ruff/validation 重跑，未发现任何 correctness、stability、semantic ownership drift、test contract、type/docstring、coupling、retained safety weakening 或 deferred scope leakage finding。

### 2.2 逐维度分析

#### 2.2.1 Root cause / semantic owner

**结论：正确，无 finding。**

直接代码证据链：

1. `dayu/host/_wait_observation.py` — token identity、state、generation 管理发布权全在同一锁内；timeout 先 invalidates token，late result 只增加 dropped count（`_wait_observation.py:204-216`、`_wait_observation.py:319-336`）。该 owner 正确，保持 no diff。
...
3. `dayu/host/durable/state.py` — `mark_wait_record_poll_abandon_timeout(...)` 完整删除（原 `state.py:2725-2801` 共 78 行），`_MarkWaitRecordAbandonTimeoutOperation` wrapper 及其 import 一并删除。保留的 `release_wait_record_poll_claim(...)` 继续拥有原子 claim release、next-observe、attempt 与 diagnostic projection。`mark_wait_record_poll_abandoned(...)` 继续服务 explicit applied/unsupported/noop lifecycle outcome 的 terminal `poll_abandoned_at` 写入。删除无误。

4. `docs/host/design.md` — 只改写 accepted plan 指定的精确句子：从 "cancelled wait 的 abandon 超时只写 `wait_abandon_timeout` diagnostic/close marker" 改为 "cancelled wait 的 abandon observation timeout 只写 poll-local transient `wait_abandon_timeout` diagnostic、释放 claim 并按 policy backoff，durable status 保持 `CANCELLED` 且不写 terminal `poll_abandoned_at`；只有 provider 显式返回 applied/unsupported/noop lifecycle outcome 才写 terminal marker"。改动精确，未扩写 policy/schema/future terminal evidence contract。

#### 2.2.2 Poll/abandon 返回计数、CAS 冲突、backoff attempt/next-observe、diagnostic、Run/Wait projection
...
| next_observe_at | 按 `_backoff_delay_seconds(1, policy)` 计算，`backoff_initial_delay_seconds=0.01`（test override） | 同左 |
...
#### 2.2.3 Late publication token/generation fence 与下一轮 observation 污染
...
- `_publish(...)` 同时检查 token identity、state（must be `ACTIVE`）、closed 与 generation（`_wait_observation.py:336-352`）；late result 进入 `dropped_count`。
...
#### 2.2.4 Authoritative typed lost、Ready/NotReady、explicit applied/unsupported/noop terminal

**结论：正确，无 finding。**

- `test_poll_adapter_lost_result_closes_run`（`test_wait_adapter_polling.py:886-903`）新增 `_RecordingPublicCommandResolver` 与 idempotency key 断言，确认 authoritative typed `WaitPollLost` 仍走 `_resolve_claimed_wait(...)` 路径、`lost == 1`、`WaitRecordStatus.LOST`、`RunStatus.LOST`。timeout code 不出现在该 branch。
...
#### 2.2.5 删除 durable primitive 的 dead import/caller/schema drift/public contract
...
#### 2.2.6 Tests 是否真正断言 owner contract、self-implementation fake、时序脆弱、test helper 误用、覆盖缺口
...
#### 2.2.7 Type/docstring/maintainability/coupling/semantic ownership drift、LLM-facing 文本和 design 真源
...
#### 2.2.8 Retained safety 是否被弱化、deferred scope 是否偷带

**结论：正确，无 finding。**

retained safety：

- late publication token/generation fence — 保持。
- outstanding capacity 与 shared close deadline — 测试 `test_supervisor_close_uses_one_shared_deadline_and_stays_closing` 通过。
- claim token CAS、release/backoff 唯一真源与 next-due claimability — 测试 `test_active_poll_claim_suppresses_second_poller_adapter_call`、`test_expired_poll_claim_allows_retry` 通过。
- authoritative typed lost 仍经 common resolver terminalize — 测试 `test_poll_adapter_lost_result_closes_run` 通过。
- explicit applied/unsupported/noop lifecycle 仍写 terminal abandon marker 且不调用 wait resolve — 测试 `test_cancelled_poll_wait_is_abandoned_once_without_resolve`、`test_poll_abandon_success_marks_row_and_clears_claim` 通过。
- invalid timeout-only symbol 零定义、零调用。
...
#### 2.2.9 Scheduler residual 是否被当前代码误修、掩盖或引入新传播
...
- 确定性 scheduler close probe `workspace/tmp/test_r05_scheduler_close_probe.py` 仍 `1 passed`，证明 close gate → clean EOF terminal closeout → promotion wake rejection 缺口仍可确定性复现。
...
#### 2.2.10 Validation/coverage/Ruff/pyright/source evidence 是否可信
...
| 门禁 | 命令 | 结果 |
|---|---|---|
| owner nodes（5 个） | `pytest test_wait_observation_runner.py::...timeout... test_wait_record_state.py::...timeout... test_wait_record_state.py::...abandon...` | `7 passed in 0.41s` |
| focused branch matrix（16 个） | `pytest 16 nodes` | `19 passed in 0.57s` |
...
| pyright changed files | `pyright dayu/host/wait_adapter.py dayu/host/durable/state.py ...` | `0 errors, 0 warnings, 0 informations` |
| Ruff changed files | `ruff check 6 changed Python files` | `All checks passed!` |
...
| no-diff audit | `git diff --exit-code ... -- _wait_observation.py waiting.py schema.py agent.py` | PASS |
...
## 3. Reviewed evidence 列表

1. `docs/reviews/wu-semantic-ownership-01-r05-s1-implementation-codex.md` — 完整读取。
2. `docs/reviews/wu-semantic-ownership-01-r05-s1-validation-continuation-codex.md` — 完整读取。
3. `docs/reviews/wu-semantic-ownership-01-r05-s1-controller-validation.md` — 完整读取。
4. `docs/host/wu-semantic-ownership-01-r05-wait-observation-state-machine-plan.md` — 完整读取。
5. `dayu/host/wait_adapter.py` — 完整 diff 与关键路径全文阅读。
6. `dayu/host/durable/state.py` — 完整 diff 与关键路径全文阅读。
7. `docs/host/design.md` — 精确 diff 核对。
8. `tests/host/test_wait_observation_runner.py` — 完整 diff 与新测试全文阅读。
9. `tests/host/test_wait_adapter_polling.py` — 完整 diff 与修改测试全文阅读。
10. `tests/host/test_phase7_waiting_integration.py` — 完整 diff 与新测试全文阅读。
11. `tests/host/test_wait_record_state.py` — 完整 diff 与新测试全文阅读。
12. `dayu/host/_wait_observation.py` — no-diff 验证 + token/fence 机制全文阅读。
13. `dayu/host/waiting.py` — no-diff 验证。
14. `dayu/host/durable/schema.py` — no-diff 验证。
15. `dayu/engine/agent.py` — no-diff 验证。
16. `tests/host/test_dispatch_scheduler.py` — R05 symbol 零命中验证。
17. `workspace/tmp/test_r05_scheduler_close_probe.py` — 确定性 probe 运行验证。
18. `docs/host/issues-implementation-control.md` — R05 gate 状态核对。

## 4. Retained safety 确认
...
| R04 config ownership / 12-field snapshot | 保留 | R04 preservation tests 通过（Controller 报告 `35 passed, 3 warnings`） |
