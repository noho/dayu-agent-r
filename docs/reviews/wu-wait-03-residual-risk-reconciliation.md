...
| Tool/provider operations observe cancellation only at checkpoints | classified-as-separate-deferred-WU | Python cannot forcibly stop arbitrary provider blocking I/O safely. This is a cross-tool provider/runtime hardening topic, not a Fins-only #92 residual. If operational evidence shows cooperative checkpoints are too slow for any tool/provider runtime, handle provider-specific preemption / timeout / process-boundary hardening under `WU-TOOLS-CANCEL-01`. |
...
- Added ordinary deferred work unit `WU-TOOLS-CANCEL-01` for cross-tool provider/runtime stronger-than-cooperative interruption if operational evidence requires it; this is not recorded as a WU-WAIT-03 residual risk.

## Result

After this reconciliation, no WU-WAIT-03 residual risk remains unclassified or active. The production poller composition validation is tracked by WU-WAIT-04 itself, not by a duplicate residual-risk row. Stronger-than-cooperative interruption for blocking tool/provider runtime calls is tracked as separate deferred work, not as a WU-WAIT-03 remainder.
