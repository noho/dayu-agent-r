...
``download_files`` 是同步聚合 facade，会临时携带仓储返回的 ``FileObjectMeta``；
调用方持久化 meta 后再转为严格 JSON。
"""
...
ARCHIVES_BASE: Final[str] = "https://www.sec.gov/Archives/edgar/data/{cik}/{accession_no_dash}/"
ARCHIVES_INDEX_JSON: Final[str] = ARCHIVES_BASE + "index.json"
ARCHIVES_INDEX_HEADERS_HTML: Final[str] = ARCHIVES_BASE + "{accession}-index-headers.html"
...
_SEC_MIN_REQUEST_INTERVAL_SECONDS: Final[float] = 0.12
# 触发限流退避的 HTTP 状态码
_THROTTLE_STATUS_CODES: frozenset[int] = frozenset({429, 503})
# 限流默认退避秒数（无 Retry-After 头时使用）
_SEC_THROTTLE_BACKOFF_SECONDS: Final[float] = 5.0
# SEC 公布口径：超过阈值后，即便恢复到阈值以下，也可能需要等待 10 分钟才解除限制。
_SEC_THROTTLE_RECOVERY_SECONDS: Final[float] = 600.0
# 限流额外重试次数（不消耗正常重试预算）
...
def _parse_http_json_response(response: httpx.Response) -> dict[str, JsonValue]:
    """解析 JSON 响应体。
...
def _json_mapping(value: JsonValue | None) -> Mapping[str, JsonValue]:
    """将 JSON 值收窄为只读 mapping。

    Args:
        value: 原始 JSON 值。

    Returns:
        mapping 值；非 mapping 返回空 mapping。
...
def _json_mapping_list(value: JsonValue | None) -> list[dict[str, JsonValue]]:
    """将 JSON 值收窄为 mapping 列表。
...
def _normalize_json_text(value: JsonValue | None) -> Optional[str]:
    """将 JSON 标量归一化为文本。
...
def pick_extracted_instance_xml(items: Sequence[Mapping[str, JsonValue]]) -> Optional[str]:
...
def pick_taxonomy_files(items: Sequence[Mapping[str, JsonValue]]) -> list[str]:
...
def pick_form_document_files(items: Sequence[Mapping[str, JsonValue]], form_type: str) -> list[str]:
...
def _build_sec_throttle_dir(workspace_root: Path) -> Path:
    """构造 SEC 全局限流状态目录。
    ...
    Returns:
        工作区内 SEC throttle 状态目录。

    Raises:
        无。
    """

    return workspace_root.resolve() / _SEC_THROTTLE_RELATIVE_DIR


@contextlib.contextmanager
def _sec_throttle_lock(lock_path: Path) -> Iterator[TextIO]:
    """获取 SEC 共享限流文件锁。
    ...
    async def resolve_company(
        ...
        for row in mapping.values():
            ...
            if str(row_mapping.get("ticker", "")).upper() != normalized:
                continue
            cik = str(row_mapping.get("cik_str", "")).strip()
            company_name = str(row_mapping.get("title", "")).strip()
            if cik.isdigit():
...
def _parse_index_header_document_entries(payload: bytes) -> list[dict[str, JsonValue]]:
...
def _normalize_sec_document_type(value: JsonValue) -> Optional[str]:
    """规范化 SEC 文档类型。
    ...
    normalized = _normalize_json_text(value)
...
def _build_file_metadata_map(
    ...
    for item in [*index_items, *index_header_documents]:
        if not isinstance(item, dict):
            continue
        name = _normalize_json_text(item.get("name"))
        if name is None:
            continue
        ...
        description = _normalize_json_text(item.get("description"))
...
def _normalize_cik_value(raw_cik: JsonValue) -> Optional[str]:
...
def _select_primary_from_index_items(items: Sequence[Mapping[str, JsonValue]], form_type: str) -> str:
