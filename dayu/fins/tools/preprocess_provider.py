"""Fins 预处理工具的独立 ToolsDiscovery provider。"""
...
_VERSION_REF: Final[str] = "fins-preprocess-tools-provider-v1"
_SOURCE_ID: Final[str] = "dayu.fins.tools.preprocess_provider"


def discover_tools(spec: ToolsDiscoveryProviderSpec) -> ToolsDiscoveryProviderOutput:
    """发现 Fins 预处理 awaiting tool。

    Args:
        spec: ToolsDiscovery 传入的 provider 显式配置。

    Returns:
        provider 输出，包含预处理 start 工具定义。
    ...
    return ToolsDiscoveryProviderOutput(
        ...
        definitions=(build_fins_preprocess_tool(ingestion_runtime),),
    )


def _source_ref() -> ToolBundleSourceRef:
    """构造 Fins 预处理 provider 来源引用。
