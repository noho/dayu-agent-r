...
- `dayu.service.fins_direct`：为 product entrypoint 提供 reusable Fins direct stream helper，覆盖 download / preprocess 的 typed request 构造、filing upload 的 `ValidatedFinsUploadFilingRequest` identity handoff、同一个 `ValidatedFinsEventStream` identity 透传和 operation-scoped cancellation；terminal 协议由 Fins validator 唯一拥有，该模块不解析 CLI 参数、不处理 stdout/stderr，也不读取 Fins storage。
...
模型装配在同一个 `compose_open_host_options(request)` owner 中分别产生三个 typed
selection：

- primary default：主 scene model hints 与 execution profile run baseline，不消费
  invocation override；
- ordinary effective：在同一默认来源上消费本次 `ServiceAssemblyOverrides.model_id`；
- compactor effective：compactor scene model hints 与 compactor baseline，不消费
  invocation override。

Service 使用 primary default 与 compactor effective 的 resolved provider、provider
model、endpoint、credential ref 校验 durable default family。同源检查在 secret header
解析和 Host 打开前完成；不一致时只报告两个 model id 与不一致字段，不回显 endpoint、
credential ref、header 或 secret。跨 family 的单次 ordinary override 合法，只改变该次
ordinary selection，不写配置，也不改变 compactor selection。compactor 自己的 runner
hint、temperature、top-p、stream 与 provider extension 仍按 compactor scene/model 配置
选择。
