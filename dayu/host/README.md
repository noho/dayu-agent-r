...
- Conversation Memory 只消费 committed canonical facts 与 accepted compact 结果；persisted accepted compact replacement 在唯一严格 typed read boundary 恢复，非法 shape、digest 或 enum fail closed，Memory projection、compact material 与 RunInputBuilder 不各自解释 nested compact JSON。assistant final answer 和普通工具证据不会自动成为 evidence-backed fact。descriptor-backed terminal answer continuity 由 Host terminal resolver 解析成 typed LLM-facing material，Memory projection 与 RunInputBuilder 不通过改写 EventLog payload 来投影回答文本。
...
不变量。当前 report 使用 fresh schema version 2，并包含 canonical terminal 同源的
compactor response summaries；两个 renderer 只消费同一个 structured report，不重新
读取输入或执行规则。
...
LLM compactor 的 fresh strict v4 input 只由 immutable `CompactionRequest.compact_input` 投影，proposal、accept、artifact 与 dispatch consumer 不从 material pack 各自重建同一输入。`compact_structure` 的单一 immutable descriptor 同源派生 prompt 使用的 concrete template / 简明字段规则、provider-native JSON Schema 与 strict parser；formal schema 只进入支持该能力的 structured-output transport，不冗余注入模型 prompt。v4 proposal 对 previous EvidenceFact 只提交 retain label，新 fact 只能引用本轮 `evidence_material`；Context Governance 从 immutable source boundary 原子复制旧 fact，并将 retained + new facts 验收为唯一 `CompactAcceptedTruthV4` / `CompactAcceptedReplacementV4`。每个 accepted fact 自带非空 canonical evidence refs，replacement 是 rolling、Memory、RunInput/reconnect 与 public Tool Trace 的唯一事实来源；aggregate refs 只是逐 fact refs 的已验证并集。represented coverage 从 accepted binding 派生，omitted coverage 由 root boundary 的 exact complement 派生。无效 proposal 只产生有界 rejection 与脱敏 repair feedback；repair 必须基于同一完整输入重产 whole proposal。多 pass 只有在 aggregate root 重新验证通过后才可提交，任何 rejected attempt 或 intermediate pass 都不能写 compact artifact、`CONTEXT_COMPACTED`、Conversation Memory 或普通 RunInput。

accepted compact 的material consumption只来自当前Session全部strict terminal所提交的`compacted_source_refs`；latest `CONTEXT_COMPACTED.event_sequence`只表示terminal ledger位置与provenance，不是raw coverage cursor。Host material owner按accepted顺序累积represented / omitted exact partition覆盖的refs，只用具有非空`run_id`的完整Run group及其唯一canonical user anchor证明已消费prefix；`run_id=None`、缺失或重复user anchor均保守进入typed projection，再执行atomic all-or-none / prefix校验。因recent floor而未进入boundary的protected group即使早于terminal也继续保留，`current_input_ref`也保持raw；rejected、failed、cancelled、stale / late no-op与tier 4/5 fallback因没有accepted terminal而不推进frontier。restart / reconnect只重读同一EventLog accepted chain，不依赖进程内cache或第二cursor。

compact recovery selection 以完整 `host_run_id` turn group 或合法 singleton block 为 atomic unit：先做 collective exclusion，再按真实 block 数和字符数执行 strict prefix budget；oversized unit 不越 cap、不拆分、不跳过，完整 raw blocks 始终保留在 canonical source snapshot。root selection 为每个 selected block 保存同序的 canonical refs 与最终 pack content digest；reactive transient pass 只能形成该 proof 的无重叠、无遗漏精确分区，且每个 pass 在 provider 前核对自身 pack。selected packer 不做文本去重；相同文本、不同 canonical ref 的事实继续保留，selected source 与 current anchor 的 canonical ref 冲突则在 pipeline/provider 前 fail closed。只有 aggregate root 可产生 durable accepted truth 或 terminal。

prediction 经 `ORDINARY`、`POST_COMPACT`、`DISPATCH_FALLBACK`、`REACTIVE_POST_COMPACT`、`CONTINUATION` 五个 stage 与三种 pressure 的十五格 closed action matrix 得到治理动作，再提交与 exact input 同源的 runner-call manifest。存在完整 policy 时，同一 transaction 按 manifest、deterministic `CONTEXT_BUDGET_EVALUATED` canonical fact、start / link transition 的顺序提交；policy 不可用时保持 manifest sizing unavailable 且不伪造预算 fact。ordinary dispatch、startup exact replay、running / waiting steer、completed / cancelled wait resume、reactive recovery 与 attachment recovery 都遵守相同 manifest-before-start 不变量；startup continuation 只从严格匹配的 source manifest 与 source budget fact 原样复用 accepted method、prediction、diagnostic、threshold 和 policy atoms，生成新的 continuation fact identity，不重新 resolve anchor，也不读取当前配置重估。Engine complete continuation 对新 candidate 在 ingest transaction 内解析 anchor，并直接记录实际 continuation input，不调用 pre-start candidate recorder。accepted compact immediate candidate（包括 reactive accepted compact）固定回退完整 conservative estimate，直到 compact boundary 后出现新的成功普通 call 才能刷新 anchor。accepted compact 后由 memory projection 消费，不直接改写 memory snapshot。proactive 与 reactive compact 都必须在 operation owner 内基于 accepted replacement 的业务文本与当前输入通过 compact 后 hard threshold 验收。
...
投影；compactor response 保留 terminal binding、actual provider/model、完整 Runner
request identity 与 provider request id availability/value，并将 accepted replacement 的逐 fact
`claim + canonical_evidence_refs` 从同一 strict semantic parser 机械投影到 resolver 与
Analyzer summary；attempt-rejected 的 fact tuple 固定为空。该公开投影不包含 selection
label、header、credential、endpoint、prompt 或 raw request/response。Analyzer 不写 EventLog、Tool Trace、payload
descriptor 或 Run / Attempt state。
...
repair feedback 同时绑定产生它的 immutable request digest 与 source-boundary digest；仅相同 root request 的 repair 复用，tier 或 boundary 改变即清空。operation 在 provider 前再次防御校验 binding，mismatch 使用既有 non-repairable failure transport，不把异常抛出 scheduler。两个 digest 只属于 Host 内部治理，不进入 repair projector 或其它 LLM-facing schema。

repair marker 与 whole-proposal action 的 LLM-facing contract 由 packaged prompt 定义，renderer 只负责精确投影和机械渲染；Context Governance 继续独占 reject 原因、实际计量值与 policy decision。item/字符 cap feedback、prompt 中的业务可读字符计量规则与 durable `policy_usage_audit` 必须从 `compaction` 的同一计量 owner、同一个 `MemoryProjectionPolicy` 和共享 Memory 字符计量原语派生；summary 只计 `text`，facts 只计各 `claim`，anchors 计每项 `title + "\n" + detail`，intents 与 references 只计各 `text`，reference `reason` 不计。canonical event、artifact 与 Memory read boundary 都校验 audit 的九项 actual 与 accepted replacement exact 相等，并校验 represented / omitted 是 root boundary 的无重叠、无遗漏精确分区；任何篡改或漂移都 fail closed。projector/renderer 不读取 policy、不复制 cap、不重算 proposal / replacement，也不增加 output schema、repair loop、材料 filter 或 verifier。
...
若 fresh read-write attachment 的首次 fixed-`now` 扫描发现目标 Attempt 尚未达到 orphan stale threshold，attachment recovery owner 会按同一 policy deadline 安排一次 bounded delayed reclassification；到期后使用新的 `now` 重做 positive orphan proof 和 CAS。每个 attachment 至多持有一个该任务，close 会取消并等待它；即使 delayed task join 异常，底层 attachment 仍在 cleanup `finally` 中释放，原始 join 异常继续透传。因此既不会在阈值前提前恢复，也不要求用户再次重连才能触发阈值后的恢复。
...
- `CONTEXT_COMPACTED`：通过 `parse_context_compacted_semantic_payload` 严格读取已提交的 v4 proposal、accepted replacement、逐 fact provenance 与 Host 派生并验证的 represented / omitted exact partition，物化 session summary、evidence-backed facts、answer anchors、forward intents、reference continuity items，并记录 latest compaction event ref；replacement 的 required-nullable `session_summary` 为 `null` 时清空既有 Session Summary Memory。Memory 只按 committed exact partition 替换对应 raw item，不从 request source boundary 或 aggregate refs 反推逐 fact 语义。
...
Memory 与 compact 的关系必须保持单向：Context Governance accept barrier 先形成 `CompactAcceptedTruthV4`；terminal permit 下由该单一真源写一个 compact artifact 与一个 canonical `CONTEXT_COMPACTED`，Memory projection 再只消费已提交事件并更新 read model。Memory 不直接写 compact artifact，不消费 operation 内存对象，不把 failed compact fallback 写成 compact 成功，也不把普通 final answer 或工具结果自动升级为 evidence-backed fact。ordinary RunInput 可以读取 memory snapshot 作为已物化 read model；pre-dispatch compact material 则由 EventLog / payload / artifact truth 构造 latest accepted compact、post-compact delta 与 current input anchor，不把 memory snapshot 当 compact input truth。latest accepted compact 的 summary、fact、answer anchor、intent 与 reference 全部文本叶子先在 Host 唯一 typed canonical projection 中规范化一次，再由同一批 atoms 同步生成 previous compacted blocks 与 typed readable view；block size/digest跟随最终 canonical text，exact validator保持严格。reopen/reconnect重新从 durable strict replacement构造 byte-exact pair，recovery只同步过滤已验证 pair；任何 consumer都不能从 proposal、aggregate refs、字符串或 snapshot重新解析或补偿同一语义。该 pair projection不参与也不改变 cumulative `compacted_source_refs` frontier。任何 Run / Attempt truth 仍只来自 EventLog 与状态索引。
