...
LLM compactor 的 fresh strict v3 input 只由 immutable `CompactionRequest.compact_input` 投影，proposal、accept、artifact 与 dispatch consumer 不从 material pack 各自重建同一输入。`compact_structure` 的单一 immutable descriptor 同源派生 prompt 使用的 concrete template / 简明字段规则、provider-native JSON Schema 与 strict parser；formal schema 只进入支持该能力的 structured-output transport，不冗余注入模型 prompt。v3 candidate 只含五类业务语义及引用，不承载 coverage、drop reason、policy 或其它治理账本。candidate 只有在 Context Governance accept barrier 完成 strict shape、source kind、语义唯一性、section cap 与预算校验后，才会形成唯一 `CompactAcceptedTruthV3`；represented coverage 从 candidate 引用派生，omitted coverage 由 root boundary 的 exact complement 派生。无效 candidate 只产生有界 rejection 与脱敏 repair feedback；repair 必须基于同一完整输入重产 whole candidate。多 pass 只有在 aggregate root 重新验证通过后才可提交，任何 rejected attempt 或 intermediate pass 都不能写 compact artifact、`CONTEXT_COMPACTED`、Conversation Memory 或普通 RunInput。
...
投影；compactor response 只公开 terminal binding、actual provider/model、完整 Runner
request identity 与 provider request id availability/value，不公开 header、credential、
endpoint、prompt 或 raw request/response。Analyzer 不写 EventLog、Tool Trace、payload
...
- `CONTEXT_COMPACTED`：严格读取已提交的 `dayu.context_compaction.output.v3` candidate 与 Host 派生并验证的 represented / omitted exact partition，物化 session summary、evidence-backed facts、answer anchors、forward intents、reference continuity items，并记录 latest compaction event ref；candidate 的 required-nullable `session_summary` 为 `null` 时清空既有 Session Summary Memory。Memory 只按 committed exact partition 替换对应 raw item，不从 request source boundary 反推 coverage。
