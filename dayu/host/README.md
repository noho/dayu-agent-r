...
普通 public opener 只通过 `CompactorRunnerBaseline` 接收 compactor 独立 `RunnerSpec`、`RunnerCallOptions` 与 compact artifact root。`open_host(options)` 在 Host 内部构造 `LLMContextCompactor`，该 compactor 使用 Host-owned prompt、禁用工具的 Engine request 和 Host-owned candidate mapper；普通调用方不能传入 compact prompt、candidate builder、quality override、raw policy ref 或低层 compactor port。`CompactorRunnerBaseline` 不进入 `dayu.host.api` 之外的低层装配语义，也不替代 `ContextBudgetPolicy`；semantic repair 次数由 `ContextBudgetPolicy.max_compaction_attempts_per_operation` 控制。
