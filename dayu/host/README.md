...
`dayu.host` 包根当前提供普通 Service-facing 的 Host public contract：`open_host(options)` opener、异步 `Host` 协议、`OpenHostOptions`、普通 Run 执行基线、Host-owned compactor runner 基线、Host-owned typed `HostEvent` 终态视图、生命周期异常 `HostClosedError`、普通 Session / Run request / snapshot 类型，以及 Host construction 的业务工具输入边界。低层同步 command handle、`start_run` / `StartRunRequest`、command-handle construction types、run-level event 补读与本地执行装配仍保留在内部模块路径，普通 Service 不应从包根依赖这些名字。
...
- context / input：`OperationContext`、`AuthorizationClaim`、`HostCallContext`、`HostMetadataEntry`、`SessionSlotRef`、`HostStreamCursor`、`HostPayloadRef`。
- opener / handle：`open_host`、`OpenHostOptions`、`OrdinaryRunExecutionBaseline`、`CompactorRunnerBaseline`、`Host`、`HostClosedError`。
...
`dayu.host.api.__all__` 包含 request、snapshot、status、error、context、stream cursor、public opener options 与 HostEvent typed view。`HostInput`、低层 `StartRunRequest`、command-handle construction types、本地执行配置契约类型与 run-level stream DTO 不进入 public `__all__`，如内部测试需要应从定义模块显式导入。Session / Run read facade 位于 `dayu.host.read_api`，低层 command handle、`start_run`、Session / Run command facade、Wait command facade 与 deferred facade 位于 `dayu.host.command`；普通 Service-facing 包根导出保留 opener / handle 入口。Host construction tooling 类型位于 `dayu.host.tooling`，由包根导出，但不进入 `dayu.host.api`。
...
`dayu.host.memory` 当前提供 Phase 9 Conversation Memory 的 typed contracts、EventLog-to-memory pure builder、deterministic digest helpers 与 RunInputBuilder memory view 所需的 repair / diagnostic contracts，不从包根导出，也不进入 `dayu.host.api`。Memory 是 EventLog 可重建的 session-level read model，不是 Host governance truth。`RUN_SUCCEEDED` 只在 EventLog payload 内保存 terminal summary ref 时，memory projection 与 RunInputBuilder inline delta 会在同一 Host transaction 内读取对应 SQLite payload descriptor，把可展示 assistant final content 写入 continuity item。
...
- EventLog append / read primitive：在调用方提供的 `HostTransaction` 内追加事件、分配全局 `event_sequence`、处理同体 `event_id` 幂等重复与异体冲突、按 cursor 补读，并支持在同一 transaction 内按 Run、event type 与 durable-neutral inline payload 文本过滤器统计 committed canonical facts。`canonical_fact` 的 inline `payload_json` 受当前 durable store 注入的正数 payload inline 阈值约束，超限内容必须使用 payload descriptor / artifact ref 与 digest 边界；descriptor 缺失、digest 不匹配或 SQLite payload row 缺失都会 fail closed。`USER_INPUT_ACCEPTED` 的完整 prompt payload 可通过 SQLite payload descriptor 保存，RunInputBuilder、dispatch 与 ingest 读取时跟随 descriptor 取得真源内容。
...
- `submit_followup_steer`：只接受同 Session active `RUNNING` / `WAITING` 目标 Run；在同一 Run 上追加 steer 输入，创建新 Attempt / execution / dispatch record 并切换 current Attempt，`RUNNING` 旧 Attempt 会以 `STEERED` 收口，`WAITING` active wait records 会被取消，commit 后唤醒 pending dispatch。目标缺失、非 active、Session 不匹配或状态非法时返回 `INVALID_STATE`，并在 `HostApiError.detail` 携带 `SteerConflictDetail`。
