...
- Run facade：`submit_followup`、`get_run`、`cancel_run`、`cancel_session_runs`；`start_run` 与 run-level `stream_run_events` 已降级为低层 / diagnostic 路径，不再进入包根普通 Service-facing contract。
