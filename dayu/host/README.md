...
LLM compactor 的 v2 candidate 只有在 Context Governance accept barrier 完成 strict JSON、source kind、represented / explicitly-dropped coverage、重复/矛盾语义、section policy cap 与预算校验后，才会形成唯一 `CompactAcceptedTruthV2`。无效 candidate 只产生有界 rejection 与脱敏 repair feedback；repair 必须完整重产 candidate。多 pass 只有在 aggregate root 重新验证通过后才可提交，任何 rejected attempt 或 intermediate pass 都不能写 compact artifact、`CONTEXT_COMPACTED`、Conversation Memory 或普通 RunInput。
