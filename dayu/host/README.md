...
proactive compact failure 可走 Host 内部 deterministic recent-window fallback：Host 先保留当前输入 anchor、stable / compact represented context 与 memory policy 的 recent raw floor，再按 reverse chronological raw turn block order 在 hard budget 内追加最近 raw turns。fallback 只写 `CONTEXT_COMPACTION_FAILED` 的结构化诊断；预算通过时从 `ACCEPTED` / `QUEUED` 直接创建普通 dispatch Attempt，不进入 `RECOVERING`，不写 `CONTEXT_COMPACTED`，不写 compact artifact，不把 failed event 物化为 memory stable fact。fallback selection 或预算估算失败、以及 selected view 仍超过 hard budget 时 fail closed。
