...
- command-handle 本地 scheduler lifecycle wiring、production concrete memory catch-up port 注入、Fins 业务语义、Service / UI 装配逻辑。
