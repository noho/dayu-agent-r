...
Context Governance 是 Host 责任。它根据 `ContextBudgetPolicy`、conservative estimator、memory snapshot、accepted evidence envelopes、当前用户输入和 compact artifact refs 进行上下文预算治理。proactive dispatch path 和 reactive engine-ingest path 构造 `CompactionRequest` 时，都会从 compact input range 的 bounded EventLog reads 中读取 `TOOL_RESULT_ACCEPTED.accepted_evidence_envelope`，并把 accepted evidence refs 与既有 evidence-backed fact refs 分开传入；range 内没有 accepted evidence envelope 时才使用显式空 evidence 输入。
