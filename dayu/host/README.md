...
RunInputBuilder 通过 memory snapshot provider 接线读取 vNext snapshot，构造 `AgentRunRequest.messages` 时必须带着 snapshot cursor 与 policy digest；snapshot 缺失、损坏或滞后超过策略阈值时，Host 进入 projection repair path。dispatch 前会追平 memory projection；snapshot 大滞后先走 rebuild / retry，重建后仍需要 repair 的 worker startup 路径按 startup failure fail closed，不能遗留 `RUNNING` / `DISPATCHING`，也不把 Run 推入 `RECOVERING`。RunInputBuilder 按固定顺序渲染 Session Summary Memory、Evidence / Fact Memory、Answer Anchor Memory、Forward Intent Memory、Trace Memory reference continuity 和 selected recent window；fact 渲染必须包含 `claim_text`、`evidence_refs`、`evidence_kind`、extraction operation ref 和 extraction event id / sequence，不能退化为 digest-only fact。
