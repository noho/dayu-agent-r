...
LLM compactor 的 strict v2 input 只由 immutable `CompactionRequest.compact_input` 投影，proposal、accept、artifact 与 dispatch consumer 不从 material pack 各自重建同一输入。v2 candidate 只有在 Context Governance accept barrier 完成 strict JSON、source kind、represented / explicitly-dropped coverage、重复/矛盾语义、section policy cap 与预算校验后，才会形成唯一 `CompactAcceptedTruthV2`；accepted candidate 的所有 label 集合与 explicit drops 均按 root source boundary 顺序规范化，使 candidate、coverage、artifact 与 `CONTEXT_COMPACTED` 保持同源。无效 candidate 只产生有界 rejection 与脱敏 repair feedback；repair 必须完整重产 candidate。多 pass 只有在 aggregate root 重新验证通过后才可提交，任何 rejected attempt 或 intermediate pass 都不能写 compact artifact、`CONTEXT_COMPACTED`、Conversation Memory 或普通 RunInput。
...
- `CONTEXT_COMPACTED`：严格读取已提交的 `dayu.context_compaction.output.v2` candidate 与 Host 验证的 represented / explicitly-dropped coverage，物化 session summary、evidence-backed facts、answer anchors、forward intents、reference continuity items，并记录 latest compaction event ref；candidate 未提供 session summary replacement 时保留既有 Session Summary Memory。Memory 只按这份 accepted coverage 替换已表示或明确丢弃的 raw item，不从 request source boundary 反推覆盖。
