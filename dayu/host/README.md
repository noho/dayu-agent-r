...
当前 ToolRuntime 仍未实现结果截断、`fetch_more` callable、完整重复工具事实治理算法、policy provider resolution、attempt tool snapshot durability 与长耗时 external job / wait record。
...
- ToolRuntime 真实执行、截断 / `fetch_more` callable、重复工具事实治理算法、ToolRuntime policy resolution。
