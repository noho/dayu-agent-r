...
- EventLog append / read primitive：在调用方提供的 `HostTransaction` 内追加事件、分配全局 `event_sequence`、处理同体 `event_id` 幂等重复与异体冲突，并按 cursor 补读。`canonical_fact` 的 inline `payload_json` 受当前 durable store 注入的 payload inline 阈值约束，超限内容必须使用 payload descriptor / artifact ref 与 digest 边界。
