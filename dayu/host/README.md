...
Context Governance 是 Host 责任。它根据 `ContextBudgetPolicy`、conservative estimator、memory snapshot、compact material pack、当前用户输入和 compact artifact refs 进行上下文预算治理。proactive dispatch path 和 reactive engine-ingest path 构造 `CompactionRequest` 时，都会生成去重后的 `CompactMaterialPack`：stable input、history input、accepted tool evidence input 与 current input anchor 分区使用 prompt-local labels，Host 内部用 provenance map 把 labels 映射回 canonical source refs / evidence refs；LLM-facing JSON 不暴露 EventLog ledger wrapper、payload descriptor、digest、cursor 或 Host provenance key。range 内没有 accepted tool evidence 时使用显式空 evidence input。
