...
Memory 的边界是 Host-neutral：它不导入 Engine / Fins / Service / UI，不表达财报业务字段，不让 assistant final answer、用户输入、working assumption 或 episode summary 自动成为 evidence-backed fact。`TOOL_RESULT_ACCEPTED` 只提供 accepted tool evidence 的 canonical source；stable evidence-backed facts 采用 compaction-gated extraction，从 accepted `CONTEXT_COMPACTED.accepted_candidate.evidence_backed_facts` 物化，并使用该 `CONTEXT_COMPACTED` event 的 id / sequence 作为 provenance。accepted tool evidence 没有通过 compact accept barrier 的 fact candidate 时只记录诊断，不合成 fallback fact；vNext reference continuity 只进入 continuity。没有 compaction 的短链路追问继续依赖 recent raw turns / older raw turns / 已有 memory，这只是 continuity，不是 stable fact。
...
deterministic recent-window fallback 是 Host 内部 compact failure 收口策略，不是 compact 成功。Host 先保留当前输入 anchor、stable / compact represented context 与 memory policy 的 recent raw floor，再按 reverse chronological raw turn block order 在 hard budget 内追加最近 raw turns。fallback 只写 `CONTEXT_COMPACTION_FAILED` 的结构化诊断；proactive 预算通过时从 `ACCEPTED` / `QUEUED` 直接创建普通 dispatch Attempt，不进入 `RECOVERING`；reactive 预算通过时在旧 Attempt 已关闭、Run 已 `RECOVERING` 的同一 Run 上创建新的 recovery Attempt。两条路径都不写 `CONTEXT_COMPACTED`，不写 compact artifact，不把 failed event 物化为 memory stable fact。fallback selection 或预算估算失败、以及 selected view 仍超过 hard budget 时 fail closed；reactive compact failure 收口为 `FAILED`，不写 `RUN_LOST`。
