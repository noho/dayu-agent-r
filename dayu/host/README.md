...
`dayu.host.memory` 当前提供 Phase 9 Conversation Memory 的 typed contracts、EventLog-to-memory pure builder 与 deterministic digest helpers，不从包根导出，也不进入 `dayu.host.api`。Memory 是 EventLog 可重建的 session-level read model，不是 Host governance truth。
