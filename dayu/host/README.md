...
这些函数保留稳定签名，但当前固定返回 `HostApiError(code=UNSUPPORTED_OPERATION, retryable=False, detail=None)`，不读取或写入 EventLog、idempotency record、purge tombstone、wait record 或 retry / replay Run。
...
当前 ToolRuntime 仍未实现完整重复工具事实治理算法、policy provider resolution、attempt tool snapshot durability、真实 HostDispatchScheduler tool-enabled composition wiring 与长耗时 external job / wait record。
...
- 完整重复工具事实治理算法、ToolRuntime policy resolution、真实 HostDispatchScheduler tool-enabled composition wiring。
