...
`utils/smoke_host_public_conversation_memory_scenarios.py` 用于人工验证同一个 Host public session 中，多组 mock 财报事实在干扰、上下文压力和长轮次下的 public answer continuity。脚本默认使用 `workspace/tmp/` 下的 fresh smoke workspace；显式传 `--workspace-root` 时才复用指定 workspace。脚本默认运行 `--suite memory-core --pressure-mode off`，表示轻量日常 smoke，不要求 compact；`--suite memory-compact --pressure-mode auto` 是压力与诊断 smoke，会运行 core 与 long 场景，并额外验收 proactive compact 是否出现 accepted EventLog 摘要。
