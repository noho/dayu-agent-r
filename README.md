...
该脚本不是 pytest，不断言模型固定回答。它会打印 Session / Run / terminal HostEvent 摘要、final answer 预览、mock tool 调用次数、DeepSeek compactor 调用次数、compact artifact 路径，并保留运行目录 `workspace/tmp/host_public_multiturn_smoke/latest` 供人工排查。脚本不输出 API key、headers、完整 prompt 或 provider payload。
