...
async def _await_test_value(awaitable: Awaitable[_RunResult]) -> _RunResult:
    """等待测试协程并保留返回类型。"""

    return await awaitable


def _run(awaitable: Awaitable[_RunResult]) -> _RunResult:
    """在同步测试中执行协程。"""

    return asyncio.run(_await_test_value(awaitable))
...
def test_sec_downloader_explicit_sec_defaults_are_stable() -> None:
    """验证 SEC User-Agent 与限流默认值由显式常量提供。"""

    assert SEC_USER_AGENT_ENV == "SEC_USER_AGENT"
    assert DEFAULT_SLEEP_SECONDS == 0.2
    assert DEFAULT_REQUEST_TIMEOUT_SECONDS == 30
    assert DEFAULT_MAX_RETRIES == 3
    assert _SEC_MIN_REQUEST_INTERVAL_SECONDS == 0.12
    assert _SEC_THROTTLE_BACKOFF_SECONDS == 5.0
    assert _SEC_THROTTLE_RECOVERY_SECONDS == 600.0
    assert _SEC_THROTTLE_MAX_RETRIES == 3
...
def test_download_files_aggregates_skipped_and_failed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _fake_stream(*args: JsonValue, **kwargs: JsonValue) -> AsyncIterator[DownloaderEvent]:
        del args, kwargs
...
def test_http_download_if_modified_branches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class _Client:
        ...
        async def get(self, **kwargs: JsonValue) -> _Resp:
            del kwargs
            return _Resp()
...
def test_http_private_methods_retry_and_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class _ErrClient:
        ...
        async def get(self, **kwargs: JsonValue) -> JsonValue:
            del kwargs
            raise httpx.ConnectError("boom")

        async def head(self, **kwargs: JsonValue) -> JsonValue:
            del kwargs
            raise httpx.ConnectError("boom")
