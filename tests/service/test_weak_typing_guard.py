"""``dayu.service`` 弱类型守卫测试。

通过 AST 扫描 ``dayu/service/`` 下所有源码文件，禁止：
...
def _service_root() -> Path:
    ...
    :returns: ``dayu/service/`` 的绝对路径。
    :raises AssertionError: Service 包缺少 ``__file__`` 时抛出。
    """
...
def test_service_disallows_weak_typing() -> None:
    """``dayu.service`` 不得出现 ``Any`` / ``object`` / 未注解 / 裸容器。"""
