...
open/follow-up terminal path、fast terminal、outbox fallback、默认 detail 输出 activity、显式 `--no-detail` 不注册 activity 输出、activity 不进入 `--log-file`、FAILED terminal 输出和 SIGINT 后 Host public cancel request；
