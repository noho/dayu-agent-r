...
- memory projection：覆盖 Conversation Memory 只消费 LLM-facing 普通对话、普通工具结果与 compact 事实，`TOOL_AWAITING` 不产生独有 memory 语义，不把 waiting lifecycle、取消、poller 或 abandon 治理文本投影给模型，并确认 durable memory consumer 不订阅 awaiting 事件。
