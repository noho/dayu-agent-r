...
- Fins direct：覆盖 reusable Fins direct Service boundary 的 download / preprocess typed request 构造、upload wrapper 到 `FinsIngestionRuntime.start_upload(...)` union API、job event stream 按 sequence 投影 progress 与 terminal、terminal event 缺失时的 bounded WARN + 合成 terminal fallback、empty read 后按 poll interval sleep 防 tight loop、unknown job / event store failure 异常透传、默认 1.0 秒 poll interval、非终态 sleep、SUCCEEDED / FAILED / CANCELLED exit mapping、durable `request_cancel(job_id)` 和非法 poll interval fail fast。
