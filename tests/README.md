...
- F06-F07 Context Governance conformance：Host owner tests 锁定 `context_governance_resolved` 只表达 dispatch permit，不复制 compact outcome；fresh v2 input/candidate、strict JSON、represented/drop coverage、section caps、重复/矛盾、whole-candidate repair、reactive aggregate-root revalidation、exhaust fallback、rolling replacement、迟到结果单 terminal，以及 artifact/EventLog/Memory/RunInput/Tool Trace 对 committed accepted truth 的同源传播。
