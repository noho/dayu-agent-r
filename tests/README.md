...
- P12.6 memory semantic smoke：`test_toolruntime_accept_barrier.py` 覆盖 ToolRuntime accept barrier；`test_compaction_contract.py` / `test_context_compact_events.py` / `test_llm_compaction.py` / `test_compaction_operation.py` 覆盖 compact material pack、prompt-local label 到 canonical provenance 映射、compaction-gated fact candidate、raw accepted tool evidence 内容注入、minimum preserve 和 summary 不建 fact；`test_compact_material.py` 覆盖 deterministic segment selection、protected current / recent raw floor、reactive frozen overflow material list、already represented block 排除、one-to-one section mapping、current input anchor 去重和 snapshot cursor lag repair-required 语义；`test_memory_projection.py` 覆盖 `evidence_backed_facts` 物化、claim_text 长度防线、no fallback facts、recent raw continuity、confirmed facts 不漂移和 old-key fail-closed；`test_run_input_builder.py` 覆盖 RunInputBuilder 对 post-compaction facts、minimum preserve continuity、memory snapshot repair 与 shared ordinary material block source 的职责；`test_dispatch_scheduler.py` 覆盖 memory projection lag repair 不关闭 Run / Attempt 为失败终态。
