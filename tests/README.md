...
Host slot key、unsupported 旧执行参数 fail fast、`--thinking` / `--no-thinking` 作为展示参数不进入旧执行参数拒绝集合、thinking event 存在时 `--thinking` 与 `--no-thinking` 输出不同、真实 `prompt.json` required context slots、mock Host public
