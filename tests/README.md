...
open/follow-up terminal path、fast terminal、outbox fallback、默认 detail 输出 activity、显式 `--no-detail` 不注册 activity 输出、activity 不进入 `--log-file`、TTY 下 thinking/activity 交错输出前收尾与 final 前运行态清理、HostApiError 结构化展示、FAILED terminal 输出和 SIGINT 后 Host public cancel request；
...
SIGINT 130、确认不启动 live event stream、terminal exit mapping、Service direct stream contract violation、SIGINT 到 operation-scoped async cancellation、
