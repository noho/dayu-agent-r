...
6-K 历史拒绝重试、hash/fingerprint/清单一致性、同公司跨日期与跨公司隔离、失败回滚、取消及再次增量跳过。
测试不会读取投资项目 workspace，也不从夹具提取财务数字。
