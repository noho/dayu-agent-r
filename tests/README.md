...
- F06-F07 Context Governance conformance：Host owner tests 锁定 `context_governance_resolved` 只表达 dispatch permit，不复制 compact outcome；`CompactionRequest.compact_input` 是 strict v2 input 唯一 projector，fresh v2 input/candidate、strict JSON、represented/drop coverage、逆序 multi-drop 按 root boundary canonicalize 后的 `CONTEXT_COMPACTED` round-trip、section caps、重复/矛盾、whole-candidate repair、reactive aggregate-root revalidation、exhaust fallback、rolling replacement、迟到结果单 terminal，以及 artifact/EventLog/Memory/RunInput/Tool Trace 对 committed accepted truth 的同源传播。
