...
- P12.6 memory semantic smoke：`test_toolruntime_accept_barrier.py` 覆盖 ToolRuntime accept barrier；`test_compaction_contract.py` / `test_context_compact_events.py` / `test_llm_compaction.py` / `test_compaction_operation.py` 覆盖 compact material pack、prompt-local label 到 canonical provenance 映射、compaction-gated fact candidate、selected raw accepted tool evidence 内容注入、vNext accepted / rejected / failed operation payload 和 whole-candidate repair 边界；`test_compact_material.py` 覆盖 deterministic segment selection、protected current / recent raw floor、reactive frozen overflow material list、already represented block 排除、one-to-one section mapping、current input anchor 去重、vNext material 对 user turn / assistant turn / evidence / previous fact view / current anchor non-citable 的直接映射边界，以及 snapshot cursor lag repair-required 语义；`test_memory_projection.py` 覆盖 `evidence_backed_facts` 物化、claim_text 长度防线、normalized open_questions / working_assumptions 去重、dedupe / bounded working set、episode summary bounded rendering、minimum preserve 覆盖过期、no fallback facts、recent raw continuity、confirmed facts 不漂移和 old-key fail-closed；`test_run_input_builder.py` 覆盖 RunInputBuilder 对 claim_text / evidence_refs stable fact 渲染、post-compaction facts、no-compaction recent raw continuity、minimum preserve continuity、memory snapshot repair、shared ordinary material block source，以及 fallback selected recent window rendering；`test_dispatch_scheduler.py` 覆盖 memory projection lag repair 不关闭 Run / Attempt 为失败终态，并覆盖 proactive / reactive compaction vNext closeout、compaction failure 后 recent-window fallback dispatch / hard-budget fail closed 不写 `CONTEXT_COMPACTED` 且不写 `RUN_LOST`。
