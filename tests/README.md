...
公共日志参数测试覆盖七个日志等级、`warn` / `warning` 两种公开写法、八个快捷项、16×16 有序
selector 冲突、root / command / action scope 合并、fresh parser namespace、`debug-stream` 与全部非 quiet
组合、quiet 冲突，以及 `log-file` 与全部合法组合的独立性。CLI main 只把 canonical level、
`debug_stream` 与默认临时日志流或显式 `--log-file` 诊断流交给
`dayu.runtime.log.configure_selected_diagnostics(...)`，并覆盖临时流工厂、日志目的地准备失败、连续调用
恢复 stderr、恢复失败仍关闭日志流，以及异常 / `KeyboardInterrupt` 路径先恢复 stderr handler 再关闭文件。
`prompt` 命令测试覆盖 CLI 参数到 Service entrypoint request 的转换、`--ticker` 到 LLM-facing `fins_default_subject` Markdown slot 的生成、非法 `--ticker` usage error、FMP 公司名增强与缺 key fallback、stable
...
open/follow-up terminal path、fast terminal、outbox fallback、默认 detail 输出 activity、显式 `--no-detail` 不注册 activity 输出、activity 不进入 `--log-file`、TTY 下 thinking/activity 交错输出前收尾与 final 前运行态清理、HostApiError 结构化展示、FAILED terminal 输出、非成功 terminal 渲染后 cursor 前进、cursor 写失败传播、SIGINT / Escape 后单次 Host public cancel request、durable accepted 与 public submit response 之间取消仍等待 accepted handoff 后发 canonical cancel、快速重复 SIGINT 仍等待 terminal 后清理退出，以及 cancelled UI 不展示内部取消 reason；
`interactive` 命令测试覆盖默认 fresh anonymous Session、prompt/interactive 共用 `cli.agent` label slot、双向同 label 复用和无 label invocation 隔离；parser/help 断言 prompt/interactive 不接受 command-local 或 root 前置 `--config`，interactive 不接受 `--ticker`，且在 runtime/Host 打开前 fail closed。真实 `interactive.json` 只要求默认 `fins_default_subject` 与 `current_time` context slots，interactive invocation identity 的 ticker 固定为 `None`。

CLI conformance F01-F05 的 owner coverage 分布在 parser/help 与 runtime-request 边界、外部 editor/PTY composer、pre-accept cancel barrier、READ_ONLY fresh attachment retry，以及真实 scene discovery 到 Host effective tool schema 的集成测试。它们共同锁定：全局配置参数不存在；显式无效 editor 保留 draft 且零 Run；Escape/CSI/Alt/paste 与双 Ctrl+C 由各自输入 owner 分类；只读拒绝不关闭 REPL 并以稳定 request identity 重试；interactive 不注册 preprocess，但 download/list/read 与独立 preprocess 能力仍可用。

上述 owner tests 不替代真实 oracle smoke。修复后 conformance refresh 必须在 clean commit 的独立环境中运行真实 CLI/PTY、真实 Host/EventLog/Tool Trace/Memory/RunInput/artifact 链，并使用真实 provider；mock/fake provider 结果不能形成 real-evidence pass，证据保存在仓库外的不可变 bundle。

TTY 路径由 `PromptToolkitInteractiveComposer` 独占整个 invocation 的 stdin：composer 测试覆盖 Ctrl+J、xterm modifyOtherKeys Shift+Enter exact bytes、普通 Enter、CSI/Alt/bracketed paste、standalone Escape timeout、Ctrl+D phase matrix、draft/cursor/history 恢复、Ctrl+C phase matrix和外部编辑器错误脱敏；真实 POSIX PTY 测试同时断言 terminal mode/echo 恢复。REPL async barrier 覆盖 pre-accept/provider/tool/closeout 的 Escape/Ctrl+C、单一 cancel waiter、第二次 Ctrl+C 只登记 exit-after-cancel且不取消 canonical waiter、后续 Ctrl+C no-op、type-ahead 跨 terminal 保留、最多一个 `QUEUE,target=None` follow-up、terminal/Enter 双序 race、accepted queued Run 恰好执行一次并完成 terminal 后才退出，以及零 STEER。`run_keys` 的运行态单字节监听只服务 prompt one-shot，不参与 interactive。
...
- logging：验证 `dayu.runtime.log` 的七个 canonical diagnostic level 到 numeric threshold 的唯一映射、默认 stderr 与显式 stream、层中立 verbose / stream-debug / bounded payload key helper、`VERBOSE` / `STREAM_DEBUG` / `WARNING` / `CRITICAL` / `QUIET` 级别契约；ordinary/stream admission 矩阵确认 `debug_stream` 只额外放行精确 stream-debug 记录，不改变普通阈值，特别覆盖 info+stream 不输出普通 DEBUG/VERBOSE、quiet 不输出 CRITICAL、quiet+stream fail closed、root 与 namespace 使用相同 gate/filter，以及 `dayu.runtime.log_levels` 只提供公共日志级别常量、不注册 stdlib logging level。
...
Download owner coverage 还包括：ticker、市场化 forms 与包含边界日期窗口在 workspace/runtime 前静态校验；显式 start-window fact 贯穿 SEC SC13 collection/retry；普通下载不执行 ticker 级 stale prune；CN 缺失期间不伪装成 provider candidate 的 skipped disposition；SEC User-Agent prerequisite 与 provider failure 分类；overwrite 单目标非删除语义；仅从既有 source 重建本地 download meta/manifest 且 provider 零调用；typed terminal summary 的互斥计数、文档明细、独立缺失期间以及成功/失败/取消退出码；CN/HK Docling 子进程在正常、失败和取消路径的 terminate/kill/close、输出校验与临时目录清理。并发与完整性矩阵以 Event/barrier 而非 sleep 控制时序，覆盖同目标双 overwrite 的 last-writer、不同目标 union、async generator 提前关闭、Phase A prefetch 后 identity/revision churn、repair unconditional transport、published/staged 的 `MISSING` / `COMPLETE` / `REPAIR_REQUIRED` 分类、缺文件/size/digest corruption、malformed SHA-256 strict failure、whole-tree repair-first revalidation，以及 multiple/unselected corruption 在 company、maintenance 或 rejected artifact mutation 前 fail closed。
...
- attachment recovery / compaction concurrency / success identity：`test_recovery_scan.py`、`test_session_attachment_registry.py`、`test_public_session_attachment.py` 与 `test_open_host_runtime.py` 覆盖 fresh read-write attachment 在 stale threshold 前不提前恢复、deadline 后使用 fresh `now` 做一次 bounded delayed reclassification、close 取消并等待任务、join 异常时底层 attachment 仍恰好 close 且原始异常透传，以及同 Run 的 recovery CAS；`test_compaction_terminal.py`、dispatch 与 Engine ingest barrier 测试覆盖 proactive/reactive 共用 terminal permit、first terminal wins、late result 在 artifact/fallback/start 前 no-op、多个 terminal fail closed；scheduler 测试覆盖同一 Session 的 wake/periodic signal 合并为单一 pre-start flight、caller cancellation 不取消共享 flight、close join 与 fresh owner 从 durable pending operation 恢复。Engine/Host compactor 测试还覆盖 `SuccessfulRunnerResponseIdentity` 的 present/unavailable、ordinary/continuation/force-answer、multi-attempt accepted/rejected/failed/repair 投影，以及 identity 与 operation/attempt/manifest/output 同源且敏感字段为零。
...
- F06-F10 Context Governance conformance：Host owner tests 锁定 `context_governance_resolved` 只表达 dispatch permit，不复制 compact outcome；`CompactionRequest.compact_input` 是 strict v2 input 唯一 projector，fresh v2 input/candidate、strict JSON、represented/drop coverage、逆序 multi-drop 按 root boundary canonicalize 后的 `CONTEXT_COMPACTED` round-trip、section caps、重复/矛盾、whole-candidate repair、reactive aggregate-root revalidation、exhaust fallback、rolling replacement、迟到结果单 terminal，以及 artifact/EventLog/Memory/RunInput/Tool Trace 对 committed accepted truth 的同源传播；proactive 与 reactive owner tests 还覆盖超限 accepted terminal 通过 payload descriptor/blob 提交，terminal、compact artifact、Memory 与 public Tool Trace response identity 继续消费同一完整 truth，digest 漂移 fail closed 且 promotion 不 fatal/hang。F10 owner matrix进一步覆盖完整 turn-group collective exclusion、真实 block item/字符 strict-prefix budget、oversized 首组不拆分/越 cap/跳过、canonical raw snapshot 保留、root selected refs/final-pack digest proof、transient pass 对 root proof 的精确无重叠无遗漏分区、每 pass provider 前 pack 校验、same-text/different-ref 保留、current-ref overlap fail closed、repair 双 digest retain/clear、defensive non-repairable failure 与 aggregate-root-only durable terminal。
- Compactor LLM-facing conformance：`test_llm_compaction.py`、`test_compaction_contract.py` 与 `test_public_compact_smoke.py` 的 deterministic matrix 覆盖 current/trace/evidence/answer 四类不可信材料边界、strict input/output schema、同源完整示例经 production parser/governance 接受、repair exact-key projection、whole-candidate replacement，以及 session summary 和四个 semantic section 的 item/字符 cap 在边界值接受、超一拒绝与多 section 同时超限反馈。测试同时断言运行期材料原文没有 production filter、业务可读 contract 字段名允许出现，而 Host/Memory/Attempt、Python 类型名和迁移术语不进入 prompt 或 repair block。

  opt-in 真实 compactor smoke 使用 production prompt renderer、runner、strict parser 与同一 policy 的 Context Governance accept；provider 顺序固定为 Mimo-first，只有 credential 或既有精确环境不可用分类才 fallback 到 DeepSeek，DeepSeek 是唯一 fallback，不触达 Gemini/Qwen。运行命令：

  ```bash
  DAYU_RUN_REAL_COMPACTOR_SMOKE=1 pytest tests/host/test_public_compact_smoke.py -q -k 'real_compactor'
  ```

  当前留存 evidence 中，Mimo 与 DeepSeek 都被精确分类为 `network_unavailable`，测试在两路环境不可用后 exact skip。没有收到非空真实 candidate，因此真实 strict parse、governance accept、cap compliance 与 injection behavior oracle 均为 `not_observed`，不能写成 behavior pass；deterministic matrix 只证明 owner contract，不替代真实模型行为观察。
...
- CLI CI Run observation：`tests/cli/test_cli_ci_run_observation.py` 覆盖 tracked `utils/cli_ci_run_observation.py` 通过物理只读 Host store 与 filtered EventLog keyset window 逐 Run 投影 `RUN_ACCEPTED` 和 shared terminal facts；reason 始终只取 `reason_json.reason`，并按 terminal-specific canonical shape 严格允许 waiting cancel 的 `mode` 与 startup orphan lost 的 `orphan_proof`，未知 extra、missing/malformed/blank/wrong type、terminal 无 accepted、missing/duplicate terminal 全部 fail closed。测试同时锁定 accepted/terminal `session_id` exact、复用 lifecycle owner 的 terminal status/public-outbox projection、`RUN_LOST` 独立分类、process exit 与 Run outcome 分离、success-required dependency stop、四类terminal summary与per-Run records逐类exact对账、upstream→dependent ordinal pure conversion、independent role可用性，以及 stop/invalid 后剩余 dependent actions 全部 not-run且只发送一次 cleanup/EOT 的确定性控制决策。final publication scanner的focused fixtures证明final completion/index进入descriptors、唯一自排除项是尚不存在的`secret-scan.json`、既有或stale report fail closed，并拒绝report path traversal与resolved root逃逸；普通repo/corpus路径不作为secret probe、实际secret exact value必命中。raw DB path owner矩阵覆盖`.sqlite/.sqlite3/.db`主库及全部`-wal/-shm` sidecar、嵌入文本与普通路径反例；scanner与filesystem snapshot consumer复用同一typed classifier，final bundle fixture证明已过滤snapshot、独立只读SQLite projection和cold JSONL Tool Trace可得到`status=complete`，同时leaf/ancestor symlink与raw文件仍fail closed。focused 命令为 `pytest tests/cli/test_cli_ci_run_observation.py tests/host/test_event_log_store.py tests/host/test_run_attempt_transitions.py tests/host/test_wait_cancel_late_result.py -q`。
