...
- transient Host→Service→CLI regression：`tests/cli/test_transient_slow_consumer_path.py` 通过真实 Host publisher/subscription、当前 Service 容量 256 relay、thinking callback、typed watcher failure、Outbox fallback 与 CLI renderer，证明 relay 先满并阻塞后 Host subscription overflow，non-retryable `DELIVERY_INTERRUPTED/TRANSIENT_MAILBOX_OVERFLOW` identity 原样进入现有 fallback，terminal/outbox 同 identity 只展示一次并在 timeout 内收口。该测试描述当前 S1 传播事实；Service relay 的删除不在本阶段声明。
