...
UF-FIX01 owner coverage 进一步固定：filing 参数在 CLI factory 与所有 non-CLI producer/observation/job 边界前以同一 typed validator fail closed 且 fresh workspace 零 mutation；production workflow 使用 fresh published-state recheck，SEC/CN/HK company/source 共享单一 publication batch，skip/转换失败不开 batch，stage/commit fault rollback 后 published tree 与 SHA 不变；shared interruptible converter 与 cancellation token 保持 identity；closed typed failure 在 pipeline result、runtime summary、direct RESULT 与 durable failure summary 同源，并拒绝未知、pathful、control 或过长字段。
