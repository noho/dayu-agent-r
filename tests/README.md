...
- provider request identity / Tool Trace correlation：覆盖 Host effective execution config freeze / restore 保留 `RunnerSpec.client_correlation_policy`，RunInputBuilder 把 Attempt `attempt_id` / `execution_id` 投影到 `AgentRunRequest`，reactive compactor request 传递 Attempt / execution 而 proactive compactor request 保持二者为空；Engine ingest 覆盖 failed terminal、recoverable failure diagnostic、context compaction request / recovery closeout、provider protocol diagnostic、iteration completed preview 中的 `provider_request_id` / `client_correlation_id` payload，以及 tool-call requested preview 的 normalized arguments digest；Tool Trace projection 覆盖 hot `trace_summary_json` 与 cold JSONL `trace_summary` 暴露 `client_correlation_id`、非文本字段 fail closed、provider request id 查询结果的 trace summary 保留客户端关联字段，以及大工具参数 descriptor 不被展开进 cold JSONL。
