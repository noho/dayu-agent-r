...
- transient Host→Service→CLI regression：`tests/cli/test_transient_slow_consumer_path.py` 通过真实 Host publisher/subscription、Service 容量 256 relay、thinking callback、typed watcher failure、Outbox fallback 与 CLI renderer，证明 Service relay 先满并阻塞后 Host subscription overflow，原 `UNAVAILABLE/retryable/slow_consumer` 诊断可见，terminal/outbox 同 identity 只展示一次并在 timeout 内收口。
