...
- durable foundation / projection core / Context Governance / Conversation Memory / RunInputBuilder / dispatch scheduler / ToolRuntime accept barrier / executor / truncation / fetch_more / duplicate governance / diagnostics / EngineEvent ingest / internal admission：覆盖 SQLite fresh bootstrap、schema version / table constraint、transaction runner commit / rollback / after-commit / write-read busy retry、rollback 失败后 runner 不复用脏连接、EventLog append / read / duplicate / conflict、canonical fact inline payload size guard 与 store policy 注入、payload inline threshold 正数精确边界、idempotency record / conflict、payload descriptor 读取与 descriptor / sqlite row 缺失 fail-closed、大 `USER_INPUT_ACCEPTED` descriptor 溢出、local artifact helper、host instance liveness、startup recovery orphan closeout / recovery dispatch / late old execution reject、recovery 多进程 live owner 不误杀、owner crash 后 public Host event stream 观察恢复 final answer、projection lag 下仍以 durable EventLog / Run / Attempt / dispatch rows 恢复、多进程 EventLog `event_sequence` 唯一递增 smoke、同 `event_id` 多进程异体写入压力测试、projection checkpoint / failure schema 与 row codec、ProjectionRunner filter / typed payload / 同事务 checkpoint advance / failure 不推进 checkpoint、minimal RunResult / Session timeline read model 投影、terminal 冲突失败、display_text 缺失 fallback、repair reset / replay / resume、Context Budget policy / conservative estimator（含 CJK 保守估算，`test_context_budget.py`）、context compact canonical payload builder / validator（`test_context_compact_events.py`）、Host-owned LLM compactor async runner path、runner timeout、length final 拒绝、untrusted material delimiter 与失败摘要脱敏（`test_llm_compaction.py`）、async compaction operation retry/failure / quality rejection retry / hard-threshold retry（`test_compaction_operation.py`）、compactor typed contract（`test_compaction_contract.py`）、fake compactor、quality check rejection matrix、compact artifact descriptor / digest（`test_compact_artifact_store.py`）、Conversation Memory typed contract 校验、Host-neutral ref 边界、evidence-backed / assumption claim status 边界、deterministic snapshot digest、memory table / CHECK / FK / index、memory snapshot 与 projection checkpoint 同事务 rollback、memory diagnostic durable round-trip、ConversationMemoryProjectionConsumer committed EventLog projection、terminal summary descriptor continuity 与 accepted compact projection、memory projection consumer-scoped reset / rebuild / catch-up failure、accepted tool evidence 到 compaction candidate、final answer / user input anti-hallucination matrix、history pool budget / stable layer budget / recent raw turn floor / episode summary 降级顺序、RunInputBuilder descriptor prompt / memory snapshot covered / inline delta / repair-required 路径、Session lifecycle、Run / Attempt transition primitive、dispatch record 四状态 schema、wait record schema / row codec / DDL CHECK / CAS helper、terminal Run / Attempt CAS refs 防御、awaiting accept 三事实原子写入 / wait record 创建 / WAITING 和 SUSPENDED 状态推进 / replay / conflict / stale execution reject、resolve_wait 原子 resume / terminal closeout / resume RunInputBuilder continuity / late diagnostic / after-commit catch-up failure tolerance、WAITING cancel、wait poller ready / not-ready / cancelled abandon / abandon 失败重试 / adapter error warning / resolve_wait 异常隔离、waiting / dispatching / worker accept refs、active cancel durable primitive、RunInputBuilder durable prompt / canonical continuity / recovery Attempt 使用同一 Run EventLog descriptor / no-tool request / 非可派发状态拒绝、scheduler pending / waiting / dispatching / worker accept / durable retry requeue / promotion catch-up failure tolerance、scheduler 默认 active registry 不跨实例共享、scheduler close 后 wake RuntimeError / close 幂等、真实 scheduler tool-enabled ToolRuntime wiring、真实 scheduler awaiting production wiring、drain loop empty / sleep wakeup 窗口、pre-accept cancel race、lane acquire timeout、worker startup timeout、active task 与 pre-consumer active handle 资源释放、proactive compact rejection retry / stale failure_reason、proactive compact vNext artifact / `CONTEXT_COMPACTED` closeout、reactive Engine overflow accepted closeout 到 recovery Attempt、reactive compact failure recent-window fallback dispatch / over-budget fail closed 且不写 `RUN_LOST`、LocalProxy single-use events / close race / Engine entry boundary、ToolRuntime effective bundle 同源 schema / callable、ToolRuntime accept key 幂等重放 / conflict / invalid Attempt / stale execution reject / reuse canonical governance / accepted tool fact catch-up failure tolerance、ToolRuntimeExecutor accepted ack barrier、rejected ack 不泄漏 raw result、大 tool result 默认透传并仅记录摘要、accept timeout bounded retry、duplicate cleanup failure 不覆盖原始返回或异常、side-effect / paid missing idempotency key guard、awaiting accepted / rejected / timeout / missing adapter / missing external job / batch stop、no-tool scope defense、mixed batch、`text_chars` / `text_lines` / `list_items` / `binary_bytes` truncation、run-scoped truncation cursor / scope token、fetch_more 普通工具注入、大 fetch_more continuation 透传、single-use / TTL / scope / token / missing cursor / invalid limit / remainder digest 错误路径、duplicate key 规范化并排除 `index_in_iteration`、同 iteration 重复调用治理、`allow` / `reuse` / `hint` / `require_justification` / `hard_stop` matrix、duplicate governed candidate 字段一致性、attempt-scoped duplicate key / in-flight owner-waiter 串行化 / durable-missing waiter 接棒 / cross-Attempt fresh request / worker restart in-memory non-durable behavior / trace scope projection、diagnostic emitter refs，以及 Engine 经 ToolRuntime durable accepted 后继续第二轮、fetch_more 经同一 ToolRuntime accept EventLog path、final answer / failed / cancelled / usage / preview data 校验 / reactive context compaction recovery / Engine awaiting confirmation diagnostic 与 accepted wait refs 匹配、Engine 工具事件 preview 边界、clean EOF / worker lost closeout、terminal duplicate promotion retry、start / follow-up admission、attach active 幂等、queue policy、idempotency、FIFO promotion、queued / pre-dispatch cancel、terminal closeout，以及 admission 多进程 durable invariant，包括同 slot ensure、同 Session active Run 唯一性、跨进程幂等、FIFO promotion、queued cancel / promotion 竞争和 EventLog sequence 全局顺序。
...
- P12.6 memory semantic smoke：`test_toolruntime_accept_barrier.py` 覆盖 ToolRuntime accept barrier；`test_compaction_contract.py` / `test_context_compact_events.py` / `test_llm_compaction.py` / `test_compaction_operation.py` 覆盖 compact material pack、prompt-local label 到 canonical provenance 映射、compaction-gated vNext fact candidate、selected raw accepted tool evidence 内容注入、vNext accepted / rejected / failed operation payload 和 whole-candidate repair 边界；`test_compact_material.py` 覆盖 deterministic segment selection、protected current / recent raw floor、reactive frozen overflow material list、already represented block 排除、one-to-one section mapping、current input anchor 去重、vNext material 对 user turn / assistant turn / evidence / previous fact view / current anchor non-citable 的直接映射边界，以及 snapshot cursor lag repair-required 语义；`test_memory_projection.py` 覆盖 `evidence_backed_facts` 物化、claim_text 长度防线、normalized open_questions / working_assumptions 去重、dedupe / bounded working set、episode summary bounded rendering、vNext reference continuity / legacy minimum preserve 覆盖过期、no fallback facts、recent raw continuity、confirmed facts 不漂移和 old-key fail-closed；`test_run_input_builder.py` 覆盖 RunInputBuilder 对 claim_text / evidence_refs stable fact 渲染、post-compaction facts、no-compaction recent raw continuity、compact continuity、memory snapshot repair、shared ordinary material block source，以及 fallback selected recent window rendering；`test_dispatch_scheduler.py` 覆盖 memory projection lag repair 不关闭 Run / Attempt 为失败终态，并覆盖 proactive / reactive compaction vNext closeout、compaction failure 后 recent-window fallback dispatch / hard-budget fail closed 不写 `CONTEXT_COMPACTED` 且不写 `RUN_LOST`。
