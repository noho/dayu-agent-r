...
open/follow-up terminal path、fast terminal、outbox fallback、默认 detail 输出 activity、显式 `--no-detail` 不注册 activity 输出、activity 不进入 `--log-file`、TTY 下 thinking/activity 交错输出前收尾与 final 前运行态清理、HostApiError 结构化展示、FAILED terminal 输出、非成功 terminal 渲染后 cursor 前进、cursor 写失败传播、SIGINT / Escape 后单次 Host public cancel request、快速重复 SIGINT 仍等待 terminal 后清理退出，以及 cancelled UI 不展示内部取消 reason；
