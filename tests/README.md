...
- web tools provider：覆盖 `dayu.tools.web` 通过当前 `ToolsDiscovery` 暴露 `search_web` 与 `fetch_web_page`、默认拒绝 private / local URL 且显式配置后才允许、搜索 optional 参数与 provider config 闭包投影、fetch truncate config 与 Playwright fallback channel / storage state config 投影、非法 URL 类型进入 Web 逻辑前失败、current success / failure outcome 投影、current `ToolTruncateSpec` 暴露，以及基于 AST import 解析确认未导入 OLD registry / truncation / `fetch_more` / UI。
